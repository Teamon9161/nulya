...
//!
//! The platform mechanics only: job objects on Windows, process groups on POSIX,
//! and the bounded wait that turns a timeout into an actual kill.
...
/// The kernel32 job-object calls `std.os.windows` does not declare; a job object
/// is the only reliable way to terminate a process TREE on Windows. Analyzed
/// lazily, so the externs never reach a POSIX link. The job carries NO limits,
/// so `SetInformationJobObject` is neither needed nor declared.
const win32 = struct {
...
///
/// `std.process.Child.kill` reaches only the direct child, which is usually not
/// the process doing the work: `bash -lc "a; b"` forks for the last command, and
/// Windows Git Bash's `bin\bash.exe` re-execs the real shell as a grandchild. A
/// survivor keeps the pipe write-ends open, so the drain never reaches EOF. So
/// both kill a tree:
///
...
/// everything the command deliberately left running the moment `deinit` closed
/// the handle.
///
...
/// both pipes to EOF.
///
/// If the OS refuses a job object, this degrades to the plain single-process
/// kill and says so once.
pub const Tree = struct {
    child: std.process.Child,
    /// POSIX: the direct child's pid, which is also the group id. Kept here
    /// because `child.id` does not survive a canceled `wait` (std 0.16 nulls
    /// `id` and closes stdio without reaping) and `killAll` must work then.
    ...
    /// child. Idempotent, and works after a canceled `wait`.
    pub fn killAll(self: *Tree, io: std.Io) void {
        if (builtin.os.tag == .windows) {
            // The job handle names the tree independently of `child.id`.
        ...
        if (self.child.id != null) {
            ...
            // A canceled `wait` got here first: std nulled `child.id` without
            // reaping, so `child.kill` would no-op and leave a zombie. SIGKILL
            // guarantees this wait returns promptly.
            reapDirectChild(self.pid);
        }
        self.reaped = true;
    }

    /// Reap a child whose `wait` was canceled before it could. Only ever called
    /// after the group SIGKILL, so the wait cannot block. EINTR is the one errno
    /// worth retrying; anything else means there is nothing left to reap.
    ...
    /// limits, so whatever the command left running keeps running.
...
///
/// The wait runs as its own task and reports through `Waiter` rather than the
/// `Select` union, because on a timeout the loser must still be asked whether it
/// nevertheless reaped the child: if the process exited in the same instant the
/// budget expired, this reports a normal exit.
pub fn waitBounded(io: std.Io, child: *std.process.Child, timeout_ms: ?u32) !Waited {
    ...
    // The wait did not complete. If the wait task is what finished, it finished
    // by being canceled — this whole call, not a timeout.
...
const Waiter = struct {
    ...
    fn run(self: *Waiter) void {
        ...
        // A canceled wait never reaped, so leave `outcome` null.
