...
test "bundled std grep: hits, smart case, glob filter, context shape, per-file cap, paging, no-match notes, gitignore, pruned dirs, oversized / binary / CRLF files, invalid regex, output budget" {
    ...
    // does not exist says so. Both are JSON-RPC errors (exit 1).
    {
        const bad = try call(alloc, io, ws, exe, ref, "grep", "{\"pattern\":\"ZQX_(unclosed\"}", 1);
        defer alloc.free(bad);
        try std.testing.expect(has(bad, "extension error [-32000]: invalid regex: /ZQX_(unclosed/"));
        ...
        try std.testing.expect(has(gone, "extension error [-32000]: search path does not exist: "));
        const missing = try call(alloc, io, ws, exe, ref, "grep", "{}", 1);
        defer alloc.free(missing);
        try std.testing.expect(has(missing, "extension error [-32602]: missing required parameter: pattern"));
    }
