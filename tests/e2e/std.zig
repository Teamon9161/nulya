...
test "bundled std: ext build compiles one binary with five tools; ext run reaches a tool by name and rejects an unknown one; version is stable across rebuilds" {
    ...
    // path that does not exist), a JSON-RPC error the CLI reports as exit 1 —
    // not "unknown tool", not an invalid response, not a crash. What the
    // refusal SAYS is std_fs.zig's business.
    {
        ...
        try std.testing.expect(std.mem.indexOf(u8, run.stdout, "extension error [-32000]") != null);
