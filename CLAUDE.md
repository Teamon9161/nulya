...
| `prompt.zig` | `Ledger → PromptIR{system_blocks, turns}` 纯投影（一个 ledger 事件一个 `Turn`，turn 不拆散；`usage` / `truncated` / `origin` 在类型里没有字段，所以不可能被投影） | `isStablePrefix` 仍是缓存不变量的可测形式；turns 借 ledger 的 slice，不比 ledger 活得久；generation == 文件（`currentGeneration` 已删） |
