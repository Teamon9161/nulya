...
| `cli.zig` | `nulya ext …` / `nulya session new\|append\|step\|events\|cancel` / `nulya config show` / `nulya src [path]` / `nulya skill list\|load` / `nulya toolchain zig` / `nulya ext api` | `ext/skill/toolchain/src/config` 经 `shell` 被模型调用；`session *` 是外部 driver 面（只有 `step` 写 session 文件）；都不是 LLM tool |
