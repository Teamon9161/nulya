...
| `cli.zig` | `nulya ext …` / `nulya session new\|append\|step\|events\|cancel` / `nulya skill list\|load` / `nulya toolchain zig` / `nulya ext api` | `ext/skill/toolchain` 经 `shell` 被模型调用；`session *` 是外部 driver 面（只有 `step` 写 session 文件）；都不是 LLM tool |
| `launch.zig` | session 启动共享件：确定性 scripted provider（`NULYA_SCRIPTED_MODE`）、`resolveDescriptor`（创建时唯一一次 credential-aware 模型解析）+ `buildFromDescriptor`（handle 只从 descriptor 建、无静默 fallback）、session id/path | CLI 与 demo 共用同一 durable 路径 |
