...
**Driver 面**（都不是 LLM tool，经 shell 调用）：`session new|append|note|step|events|cancel|rebind|outcome|list|prune` · `task run|list|status|wait|kill|retarget` · `ext *`（含一次性的 `ext migrate`） · `config show|refresh` · `journal append|read` · `src` · `skill list|load` · `remote serve|check|ls`。`session step --stream` 是行协议，`--gate` 是每个 tool call 的一票否决。TUI（顶层 `tui/`，Bun + OpenTUI）是第一个完整 driver；`drivers/goal.{sh,ps1}` 是最小的那个（各 ≤ 70 行、都不解析 JSON）。

**两条 journal**（append-only，持 `<file>.lock` 写、读端忽略残尾）：`tool-usage`（证据，内核零读者）· `session-outcomes`（评判，没有行 = unknown ≠ failure）。
