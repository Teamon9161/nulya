...
| `loop.zig` | 一次 step：freeze snapshot → model.step → 串行执行 batch → 一条 tool_results | 取消时补齐整批（三种 marker）；`max_tokens` 截断的 turn 不执行、torn 参数换 `{}`、marker 批次关掉（`StepOutcome.stop_reason`）；`completeInterruptedToolBatch` 修复上次残尾 |
