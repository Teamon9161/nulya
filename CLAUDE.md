...
| `cli.zig` | `nulya ext …（含 --user）` / `nulya session new [--parent] [--with] [--pin]\|append\|step\|events\|cancel\|outcome\|list` / `nulya config show` / `nulya src [path]` / `nulya skill list\|load` / `nulya toolchain zig` / `nulya ext api`  `ext/skill/toolchain/src/config` 经 `shell` 被模型调用；`session *` 是外部 driver 面（只有 `step` 写 session 文件）；都不是 LLM tool |
...
zig build e2e       # tests/e2e.zig：真实二进制的 extension 闭环 + 自造 + 按 pin 晋升
