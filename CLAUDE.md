...
| `composition.zig` | session 开始冻结 tools / skills / system prompts / 成员 extension 的版本（`FrozenExtension`） | 两根轴分开：**版本冻结** ≠ **pin**（pin 只指 native 工具面）。成员解析三条路（discovery / `--with` / header 冻的 `active`）**一律硬失败**：坏掉的 active 版本 → stderr 指名 + `ActiveExtensionBroken`（host fault 照原样传播）；pin 解析不到 → `PinNamesUnknownExtension` / `PinToolNotDeclared`；`max_tools` 含 builtin |
...
| `extension/build/{build_ext,toolchain,templates}.zig` | `nulya ext build`：冻结 snapshot →（`bin/` entry）`zig build-exe` frozen `src/main.zig` /（`src/` entry）脚本直接冻结不编译 → seal | 内嵌 Zig 0.16 由 `-Dembed-toolchain` 门控；脚本 build 不需 zig |
...
| `tool_stats.zig` | `.nulya/tool-usage.jsonl` `{v:1,tool_id,ok}`：append + 读回 + `aggregate`，**内核里没有读者** | 只是 evidence：晋升 = 人 / evolution session 写一条 pin（DESIGN §5.5、PLAN §3.7） |
...
| `cli.zig` + `cli/{common,session,ext,config,skill,src,toolchain}.zig` | `cli.zig` 只有 `dispatch`；一个动词族一个文件，共用件在 `cli/common.zig`。`nulya ext …（含 --user）` / `nulya session new [--parent] [--with] [--pin]\|append\|step\|events\|cancel\|outcome\|list` / `nulya config show` / `nulya src [path]` / `nulya skill list\|load` / `nulya toolchain zig` / `nulya ext api`  `ext/skill/toolchain/src/config` 经 `shell` 被模型调用；`session *` 是外部 driver 面（只有 `step` 写 session 文件）；都不是 LLM tool |
