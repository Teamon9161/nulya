...
| `ledger.zig` | 6 种事件、deep-copy 所有权、durable 文件（typed header + `seq` JSONL）、跨进程 inbox | 唯一写口是 `append`；一文件 = 一 generation = 一缓存域；单写者由 `<id>.lock` 独家强制；投递 id 就是 exactly-once 键 |
