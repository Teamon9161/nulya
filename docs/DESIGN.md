...
都是普通 extension，走 §7.4 同一条 build → activate 路，**没有一个是内核层**；六个有 runtime 的都按 §7.3 那一种 wire 被调用（manifest 里没有一个字提它）：**八个里只有 `guide` 写 `apply: "auto"`**（一个 skill 目录条目，常驻才有意义；`ext sync --activate` 拒绝替用户激活它，所以打开这个开关仍是人显式 `activate` 的那一下），其余七个不写（= `manual`）、默认不在任何 composition 里（成员是 config `[extensions] with` 或 `session new --with`、工具面是 pin，§5.1 那张表，全是用户或 driver 的决定；`activate` 只说 `<id>` 指哪个版本），随 checkout 到达的 store 照过 §9 的 trust gate。
...
| `guide` | data | skill | 用户 `--user` 装一次，每场 `<available_skills>` 多一行（`apply` 的第一个真实 consumer：写着 `"apply": "auto"`，activate 之后不用再往 config 里加一行；`sync --activate` 拒绝替用户按下这个开关，所以决定仍是人的） |
