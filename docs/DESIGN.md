...
nulya session new [--profile P] [--model ID] [--parent <id>:<seq>] [--with <id>[@<version>]]… [--pin ext:<id>/<tool>]…
          ...
          | discard <id>                                 ← **唯一一个删 session 的动词**，且只删得掉什么都没记下的那种（header 一行、
                                                           ...
                                                           这里检查与删除全程持两把租约（deposit lease 用 non-blocking：「有人正在投递」
                                                           是答案不是队列），所以「什么都没握着」这句话在据此动手的那段时间里是真的
                                                           ——而它拿的那把是 **inbox 的**租约，投递者一个不落地都持它（§3.4），不是
                                                           两条命令之间的约定：supervisor 送回的 `task_finished`、`ext activate` 的
                                                           capability note，与一条排队的 turn 一样是「别动这场 session」的理由
                                                           **exit 0 只有一个含义：它没了，且是这条命令删的**；其余（没有这场 / 有事件 /
                                                           有排队的 turn / 有人在写 / 有人在投递）一律 exit 1 + 一句理由
