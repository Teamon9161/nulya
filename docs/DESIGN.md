...
**`task list` 的第五个投影值 `unreachable`**：这台 host 问不到那台机器。它**不是 `lost`**（那说的是 supervisor 死了，判据是对面文件系统上的一把锁，远端不问它）也**不是 `done`**——什么都不知道，而任务多半还好好跑着。`wait` 撞上它当场结束并点名那台机器（挂死不是答案）。`--json` 因此多一列 `machine`：`log` 那一列此时是**别的机器上的**路径，本机读者打不开，这一列说明为什么。
