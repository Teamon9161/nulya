...
**读它的人分两种，问的是同一个问题的两个时刻。** 写者在 step **里面**问，那里每条事实都已经 committed，答案就是 `effectiveIdentity`。任何**在 step 外面**的读者（`session append --image` 与 `session rebind` 的两道门、`session new --parent` 继承什么、前端）还多一个地方可以藏着身份：**inbox**。一条投递了还没排干的 `model_rebind` 与已经 append 的那条一样是定了的事——没有东西能收回它，而下一个 step 边界就会应用它——所以只看 committed 事件的读者，答的是一个这一场**马上就要离开**的模型。这些读者一律问 `ledger.scanSession`（header → committed → pending，外加"这一场有没有图片"同一次读出来；它不开 ledger，因为 `openDurable` 要拿写者租约，而它们每一个都必须在 step 跑着的时候能工作）。**它先读 inbox 再读 ledger——与排干的顺序相反，而这正是因为它与写者并发**：drain 搬一条事件的动作是「先 append 进 ledger，再删掉那个文件」，所以先看 ledger 后看 inbox 的读者会撞上一个交接窗口，一条比它自己还早就定下的事实在两处都不在（读完文件之后才 commit、列目录之前就被删）。反过来读，凡是这次扫描开始前已经定下的事实，两趟里至少有一趟看得见。代价是次序：ledger 里那条 rebind 可能**比 inbox 里等着的那条更旧**，所以**还在等的那条胜过已经 committed 的**（它必然排在全部已 committed 之后）——**但只在它确实还在等的时候**。这个限定词承重：inbox 那一趟是一个文件一个文件读的，并发的 drain 可以在两次读之间把**更晚**的那条 commit 掉并删除，于是这次扫描手里攥着较早的一条、而更晚的那条它哪儿都没看见；此时若一口咬定「等着的赢」，答出来的模型已经过时两条事实了。裁决靠投递 id 本身：**ledger 里若把它记成了 `origin`，说明 drain 已经走过这里，ledger 最后那条 rebind 才是更新的真相**（顺带也就把「崩在 append 与 delete 之间留下的残余文件」白拿地答对了）。图片没有这个问题——它只增不减，两趟都往上加。相等判据也只有一处 `ledger.identityEqual`：**整个 `Identity`，profile 也在内**——descriptor 说"哪个模型、走哪条 wire"，profile 说"用谁的凭据够得着它"，只差 profile 的两个身份是两种被回答的方式，比得少了就会把一次真的切换读成 no-op。
...
- **模型身份创建时冻结、resume 不可变（physics #2/#5）。** 模型解析**只有一处决定**：`launch.resolveDescriptor(prov, env, profile)` 在**创建**时把 profile 解析成 `model_identity`，运行用的 handle 也**只从这个 descriptor** 构建（`launch.buildFromDescriptor`）——所以"实际跑的" == "header 冻结的"，不存在 fork。`resolveDescriptor` 是 **credential-aware** 的：openai profile 若 `api_key_env` 在环境里解析不出 credential，创建时就冻结成 scripted（因为那正是会跑的东西）；此后 config 改动**永不**改变已有 session 的模型。resume 时 `session step` 用 header 的 `model_identity` 重建**恰好那个**模型，只从 `api_key_env` 重解 credential——**不存密钥**，也**没有静默 fallback**：openai session 的密钥不在了就 `MissingCredential` 显式拒跑。**durable credential 只以 `api_key_env` 引用**；inline `api_key` 无法在 resume 时从环境恢复（否则又让 session 依赖 mutable config），因此不参与 durable openai 身份。`provider==""` 的旧 header 当 scripted 处理。
  ...
  **唯一一种合法的改变是 append 一条 `model_rebind`**（§3.1、§9.5 的 `nulya session rebind`，goals/model-rebind.md）：header 仍然不可改写，冻结点变成一串，而"跑的 == 冻结的"逐字不变——每条 rebind 冻的同样是**已解析的** descriptor，`session step` 用它重建**恰好那个**模型、只重解 credential，credential 不在了同样 `MissingCredential` 显式拒跑。三道门都在壳层、都在投递之前（凭据 · 已有图片时新模型必须主张 vision · step 边界由 inbox 排干天然保证），内核核心不知道它们存在。**vision 那道门是两条命令共同守的一条规则**（`--image` 拒绝当下模型看不见的图片、`rebind` 拒绝看不见已有图片的模型），而两条都是「先读、后投递」——同时跑就会双双读到旧状态、双双放行，落地的恰好是它们要拒的那一对。所以两者在 `<id>.inbox/.deposit.lock` 上排他串行**整个读—投递**（`ledger.acquireDepositLease`；锁在 inbox 里而不是 `<id>.lock` 上——后者是 step 的写者租约，而这两道门必须在 step 跑着时能工作）。**`session append` 一律拿这把锁**（不只带图片时）：投递 id 是按「inbox 里已经有什么」铸出来的，两条并发的 append 不串起来就可能拿到同一个队列位置；而 `session prune` 也要靠它，才能保证自己不是在别人的 check 与 deposit 之间把这场 session 抽走——两条投递命令因此都在**拿到锁之后**再确认一次 session 文件还在。代价说在明处：换 provider = 前缀缓存作废，而 rebind 之前的 reasoning 从此不再回放（§3.1）。
...
- **一场 session 的旁车清单**（都由 id 派生，都不是 session 文件本身）：`<id>.lock`（单写者租约）· `<id>.inbox/`（跨进程事件投递，兼放它自己的 `.deposit.lock`——**每一个投递者**都持的那把锁，§3.4）· `<id>.cancel`（取消标记）· `.nulya/scratch/<id>/tool-output/`（`emit` 的落盘，§4）· `.nulya/scratch/<id>/tasks/t<N>/`（后台任务，§6.1：`status.json` / `output.log` / `.lock` / `kill` / `notify`）。后两者同在 `scratch/<id>/` 下是有意的——一场 session 的全部副产品是一棵子树，`rm -rf .nulya/scratch/<id>` 一次清干净。
...
- **投递持 inbox 自己的租约。** 写 inbox 的每一个人都先拿 `<id>.inbox/.deposit.lock`（`ledger.acquireDepositLease`，缺省 `depositEvent` 自己拿；只有已经握着它跨越「先读后投」的调用方走 `depositEventLeased`），因为这条规则是 **inbox 的**、不是某两条命令之间的约定：`session append` / `session rebind` 的读—投递要成为一次动作，`ext activate` 的 `capability_note` 与 supervisor 的 `task_finished` 同样是「不要在这时候把这场 session 抽走」的理由。锁属于 substrate 而不是某个 CLI helper，新的投递者才不必**记得**遵守它。配套的另一半：**每次投递都在锁下重新确认 session 文件还在**，不在就 `NoSuchSession`、一个字节都不写——`session prune`（`ledger.pruneSession`）因此可以在两把租约下把「什么都没握着」这句话一直握到删完为止（§14），而它删掉之后才到达的报告不会在没人排干的 inbox 里留下一条 durable 事实。`task retarget` 把一条未排干的报告 rename 进另一场的 inbox 时守同一条规则（目标场的租约 + 同一次确认）。锁顺序：没有任何地方先拿写者租约再拿它（`step` 从不投递），唯一同时握两把的 `ledger.pruneSession` 先拿它、写者租约用 non-blocking，两个方向都等不上对方。
- **inbox 应用 exactly-once（投递 at-least-once）。** 单个排干事件把 inbox 文件名作为 `origin` 落到 ledger 行上；合并的用户批次把全部文件名写进 `origins`。`Ledger.origins` 集合从两列重建。若"append 进 ledger 成功 → 删 inbox 文件"之间崩溃，文件残留，下一次排干发现对应 origin 已在 ledger 里就只删不再 append——即使只删掉合并批次的一部分就崩溃，其余文件也不会被重复应用。因此重复投递（同名文件再现）与崩溃都不会重复应用。`capability_note` 额外按内容（id+version）去重，任何名字下再宣告同一版本都是 no-op。**因此文件名就是投递 id，选名字 = 选"同一件事再说一遍"还是"另一件事"**：想幂等的投递者取一个确定的名字（note 用 `note-<id>-<version>`），想每次都是新事实的（`user_text`、`model_rebind`）取 `ledger.freshDeliveryName`（时间戳 + nonce，唯一实现）。它承诺**两件都承重的事**：**每次都不同**（名字就是 exactly-once 键；**按构造成立的作用域是这个 inbox**——戳是跨过**还在等**的那些铸出来的，已排干的名字已经不在目录里了，对它们靠的是 128 位随机尾巴的抗碰撞，不是证明；更强的承诺要第二份 durable 状态，而「活得过排干的状态」正是这里刻意没有的东西），以及**排在这个 inbox 里同前缀的每一个既有名字之后**——因为排干是按文件名序的，名字不只是身份，**它就是队列位置**。而队列位置光靠墙钟给不了（时钟会重复、会回拨，那时决定顺序的是随机尾巴），所以铸名时**读一眼 inbox、跨过它见到的最新那个戳**；已经 committed 的事件不需要这份小心——还在等的一律排在它们之后，`drainInbox` 与 `scanSession` 都建立在这句话上。这份保证的作用域正好是**顺序有含义的那个集合**：两条排队的消息按这个次序合成一个 turn，两条排队的 rebind 里最后那条才是这一场的去向。不防的两件：两次铸名互相竞争（那本来就不是谁的意图——而真正在意的两条命令 `append --image` 与 `rebind` 本来就在 deposit lease 上串行），以及**跨前缀**的顺序（那是前缀的序不是时间的序，纯 cosmetic——rebind 根本不是 turn，note / task_finished 落在消息的另一侧也只是两个 turn 擦肩）。为什么不把顺序另开一条通道、让名字只当 id：那要第二份 durable 状态，而一个「每次排干就清空」的目录上的计数器会**重用编号**——重用的名字正是「同一件事再说一遍」的静默失败（agent 包的 mailbox 吃过这个亏）。**投递的大小有上限**（`ledger.max_inbox_event_bytes`，32 MiB）且**在投递处拒绝**：一条收下了却读不回来的事件不是一次被拒的命令，而是一条让此后每个 step 边界都排干失败的 durable 事实——所以「收下的必定读得回」这条不变量守在唯一的写入点上，读的两处（`drainInbox` / `scanSession`）用同一个数。取错了是**静默**的——第二次投递会在下一次排干时被当作已应用而直接删掉，永远到不了 ledger。单写者由 `<id>.lock` 租约独家保证——`persist` 不再做长度核对：那既非并发原语（租约已挡住第二写者），也非完整完整性检查（同尺寸覆写发现不了），ledger corruption 由 replay / seq / JSON 校验负责。读者（`session events`）只读原始行、不打开写句柄。排干只在 step 边界发生，任何投递事件绝不插进一条 batch 中间（§4 的 batch 不变量成立）。`parent` 是 fork / compaction 的机制（compaction 本身未实现，见 PLAN §3.4）。
...
- **`NULYA_SESSION` 是路径，`NULYA_SESSION_ID` 是身份，`session step` 两个都发布。** 它们从前是一个变量，而"这一场叫什么"与"这一场的文件在哪"是两件事——工作区可以住在别的机器上（§8.2），那里有前者而根本没有后者。所以要**文件**的读者（就是上面这一条：往 inbox 投 note）读 `NULYA_SESSION`，只要**名字**的读者（`nulya session outcome` 的 `by:`、usage journal 的 `session` 列与 `nulya task …` 的缺省场次，都经 `cli/common.zig` 的 `envSessionId` 一处读；`extensions/std` 的 freshness 键）读 `NULYA_SESSION_ID`，而后者是唯一一个过通道的（§8.2）。（`extensions/handoff` 曾为文件名读它；提议改成随 ledger 走之后它谁都不读了，§11。）
...
后台命令与前台命令**跑在同一台机器上**，两种搬法各自成立：exec target（§8.1）下 supervisor 仍是 host 进程（它持租约、排日志、往本机文件投递事件），只是 `startShellTask` 把本场的 spec 作为 `--env <spec>` 传给它；**`remote:` 一族（§8.2）下连 supervisor 都在对面**——那台机器上起一个同样的 `nulya task supervise`，log 与 status 在对面的工作区，报告由 host 侧任何一个问它的动词翻成 `task_finished`。两条路上 `nulya task run` 都从那一场的 header 读同一个字段并建同一个 environment（`launch.sessionEnvironment`）——一个 session 里两个入口不会给出两个答案。
...
- **`--target <arch>-<os>` = 为另一台机器编译，产出的就是同一个包的另一个版本**（`ext build --target`，`extension/target.zig`）。**内核里什么都不用加**：`target` 从第一天起就在 compiled 版本的 id 与 seal 里，所以 per-target 不改 store 布局、不改 seal schema、不加 manifest 字段——它只是让那一列不再恒等于本机。
  - **词形是两个词，不是 zig triple。** 闭集 `x86_64|aarch64` × `linux|windows|macos`，与 seal 那一列**逐位相同**——它是 donor 匹配（上一条）与将来 `exec_version` 反查（PLAN、goals/remote-env.md §3.1）共用的那把键，收一个三词的 triple 就会让 `x86_64-linux-musl` 与那台机器本机建出的 `x86_64-linux` 变成两个不同的版本。abi 因此是**这里选的**而不是问来的，且**每一次 build 都走它**（交叉与本机同一条路，见下一条）：`linux → musl`（静态，不挑发行版）· `windows → gnu`（Windows 上本机 zig 用的就是它）· `macos → none`（zig 自带的 libSystem stub，不需要 SDK）。认不出的词整个拒绝并列出词表——一个被静默改写的 target 会产出一个 id 说甲、字节是乙的版本。
  - **两个词决定编译 invocation，不只是描述它**（`target.effectiveTriple`，唯一一处）：一个 id 命名的是"这些包字节 + 这个 target + 这个 compiler"，所以记同样两个词的两次 build 必须是**同一条编译命令**。因此 **host build 也显式传 `-target`**——传的是本机那两个词对应的 triple，与别人为这两个词交叉建时**逐字相同**（同一个 abi、同一个 baseline cpu）。**例外只有一个**：本机那两个词若不在闭集词表里（如 `riscv64-linux`），保持 native——`--target` 拼不出它的词，所以不存在能与它相撞的交叉产物，而那台机器必须仍能为自己 build。
    - **为什么必须收口**（原来 host build 保持 native、cross build 选 musl，两者都记 `x86_64-linux`）：那个论证只覆盖**一个 store**——`findMatchingVersion` 会找到已经在那儿的那份、一个字节都不编译，所以单 store 内确实不会有两份字节争一个 id。但 **id 会离开 store**：`ext push` 的 `store-stat` **只按 id** 答 `held`，而 `exec_version`（§3.4）指定的正是"这个 id 就是那台机器上服务这次调用的实现"。于是远端本机建的（glibc、动态链接）与 host 交叉建的（musl、静态）可以同 id、不同字节、**行为不同**——收敛之后这个洞关上。
    - **id 不变、零 churn**：id 哈希的是**两个词**不是 triple，所以既有版本的 id 一个都没变，已 seal 的老版本照旧有效（各自 seal 记着各自的 `binary_digest`）。变的只是**新产出**的字节：linux 从 glibc 变 musl（静态，对"推到别的机器"这个用途更对），且显式 `-target` 同时把 cpu 从 native 特性降到 baseline（跨机器可复现性变好）；Windows 本机本来就是 gnu、macOS 是 none，变化只有 cpu baseline 那一半。**老 store 里已存的 glibc 版本会被 `findMatchingVersion` 继续复用**（同 id 已在就不重编）——pre-release 接受这一点；要一个干净的 store 就 `ext prune` + 重 build。
  - **两个词仍然说不出的那件事**：**字节**。同一个 compiler、同一个 target 在两台机器上仍可能产出不同的字节（编译器的非确定性；实测 Zig 0.16 的 PE 输出每次链接都换一个 COFF TimeDateStamp 与一个 debug GUID，ELF 则逐字节可复现）。id 诚实的语义因此是"一个 id 一个**编译 invocation**"——一个行为等价类，不是一个字节串。安全性照旧不靠这个：每台机器对**它自己持有的字节**重验 `.sealed`（donor 复制、`ext push`），没有谁会跑一份自己没验过的字节。反过来把 abi 或 host 塞进 id，买到的是没人提过的区分，付出的是"一个 id 一个答案"这条整个 store 赖以成立的性质。
...
**两半分开：名字在 host claim，命令在它该跑的机器上跑。** `environment.claimTaskSlot`（独占 mkdir 取第一个空 `t<N>`）与 `environment.spawnSupervisor`（plain spawn / stdio null / `DetachedStdio` / POSIX `pgid=0` / Windows `create_no_window`）是两个共用件：local backend 与 `nulya remote serve` 起 supervisor 用的是**同一段** spawn，而名字**永远**由 host 分配——它是 ledger、回执与每个 `task` 动词说的那个东西，而 ledger 在 host。`SupervisorSpawn` 上 `--session <file>` 与 `--task <sid>/t<N>` **恰好二选一**，这个选择就是"报告投进那个 session 的 inbox"与"报告留在 log 旁边等 host 来取"的分界（§8.2）。
...
**`session new --parent` 继承它**（goals/review-fork-remote.md §1）：`--env` **缺席**时一个 fork 的 `environment` 与 `remote_workspace` 一起从父 header 的冻结值取——它是创建时的身份事实，和 `model_identity` 同一类，不是"新 session 边界该重新 resolve 的 composition"（`--with` / pin 才是那种东西）。`extensions/compact` 因此不必自己抄这两列：内核已经知道，抄一遍是"一个决定在多层各做一遍"。**唯一覆盖入口是显式命名 `--env`**（哪怕是 `--env local`，它就是归一成 `""` 的那个值）——命名了就完全按 argv 取，父场的两列一概不参与；换机器还沿用父场的路径才是猜。单独给出 `--workspace`（不带 `--env`）只覆盖目录那一列，机器仍是继承来的。继承来的值走与 argv **完全同一条**校验（`execTargetRefusal` / `legacySshHint`），拒绝文案点名这个值来自哪一场父 session。
...
`remote:ssh:` 缺省仍是完全非交互的 `BatchMode=yes`。只有显式给 `--ssh-password-stdin` 时才改成 `BatchMode=no` + `NumberOfPasswordPrompts=1`，并强制走固定 askpass helper：CLI 从 stdin 有界读取一行、调用结束前覆零；密码由 host 进程内的回环 one-shot broker 交给同一 nulya 二进制的 askpass 启动模式。密码不进 SSH stdin（那里始终是 framing）、argv、env、文件、header、ledger 或日志；env 里只有回环 endpoint 与随机一次性 capability。`remote check` / `remote ls` / `session new` / `session step` 都认这一个 transient flag，其中 `step --gate` 先消费密码行、随后同一 stdin 照常读 verdict。StrictHostKeyChecking 完全不改，未知 host key 仍由 OpenSSH 按原配置拒绝或询问失败，绝不自动接受。
```
...
**四个动词都搬走了。** `runShell` / `runExtension` / `putWorkspaceFile` / `startShellTask` 全部过通道。最后一个是 Phase 4：对面起一个**同一个二进制的 `nulya task supervise`**（同一个角色、同一个 `Tree`），log、`status.json` 与租约都在对面的工作区，所以后台命令与前台命令在同一台机器上，而且**活得过这条通道**（agent 死了任务不死——它有自己的进程组 / 没有 console）。`task supervise --env remote:…` 仍然拒绝——supervisor **包**一条命令，而 remote spec 是一条通道。
...
**远端那台机器的 workspace store 由它自己的门管**（§9 的 trust gate，同一条规则的另一台机器上的实例）：`.nulya/extensions` 在对面同样是 checkout 内容、同样是第一优先 root，所以一个随 clone 到达远端的 store 本可以 shadow 掉 host 明确 `ext push` 进那台机器 user store 的版本，而它从未被任何人看过一眼。于是 `remote serve` 在解析任何东西**之前**，对这次调用的 cwd 跑与 host 逐位相同的判据（`launch.occupiedWorkspaceStore` + 那台机器**自己**的 `<NULYA_HOME | ~/.nulya>/trusted-stores.jsonl`）——**判据与记录都在持有字节的那一侧**，与 entry 变体、`.sealed` 同一条理由。不同的只是**拒绝的形状**：这里没有一场 session 可以拒掉，所以它是**一次失败的调用**（点名 store 路径 + 指路在那台机器上 `nulya ext trust`），与"对面没有这个版本"逐位同形——通道不死、session 照常、模型读得到。答案按 cwd 在一个 serve 进程内记一次（一条通道一个工作区），寿命就是那个进程，所以对面跑完 `ext trust` 之后**下一条通道**即生效，没有需要作废的缓存。门只管 workspace root（user store 与 `extensions.paths` 对面同样够不着 checkout，§7.2/§9.5——这也正是 `ext push` 落在 user store 的理由）；`run-shell` 不过门，与 host 上 `shell` 从不过门一致。
...
**远端 agent 永不需要模型或 tool credential**（§9 的直接推论，也是这条设计的卖点；SSH transport 自己的认证是 host 侧 transient 输入，见上面的 askpass broker，不进入帧协议）：模型连接留在 host，对面只执行。协议里**没有能装 credential 的字段**，host 从不转发自己的 env map，而传输子进程拿到的是 `environment.sanitizedChildEnv`（`isSecretKey` 剥过、加了 `NULYA_EXE` 的那一份——**同一个函数，两台机器各跑一次**：agent 在对面用它给自己的子进程建环境）。`NULYA_SESSION` **不下传**：那是 host 上一个文件的路径，发过去就是一句假话。**下传的是 `NULYA_SESSION_ID`**——这一场的身份，在哪台机器上都成立。这两个变量从前是一个：远端化只是把它掰开，于是**只要 id 的读者**（`extensions/std` 的 freshness 门、`nulya session outcome` 的 `by:`、usage journal 的 `session` 列、`nulya task` 动词的缺省场次）在对面照常工作，而**真要一个文件的**那些（`ext activate` 投 capability note）仍然只在 host 上拿得到路径。（`extensions/handoff` 曾在这张读者清单上——它要 id 只为拼文件名；提议改成随 ledger 走之后它谁都不读了，§11。）
...
**后台任务：命令在对面，名字与投递在这边**（Phase 4，goals/remote-env.md §6.7）。`shell {background:true}` 与 `nulya task run` 在远端场上照常工作，分界只有一条：**名字**（`<sid>/t<N>`）是 ledger 说的东西而 ledger 在 host，所以 host claim 它（`claimTaskSlot`，与本机同一段代码）；**log / `status.json` / 租约 / kill 标记**在命令旁边，也就是对面。**路径不过通道**——三个动词（`start-task` / `task-poll` / `task-kill`）带的都是那个全名，两侧各用 `cli/task.zig` 的 `taskDirRel` 对着自己的工作区拼路径（`launch.sessionTasksDir` 是那一条规则，跑两遍）。

**报告是被取回来的，不是推回来的**，因为协议里没有 unsolicited 帧（规则 1），而对面那个 supervisor 也投递不了——session 文件在 host。所以它把报告写成 `<task dir>/report.txt`（**在写 `done` 之前**，与本机"先 deposit 后写 done"是同一条承重顺序：谁看见 `done`，谁必须已经看得见结果），而 host 侧读它的一端也要守同一条顺序的另一半——**一次 poll 可能恰好落在那两次写之间**（report 已经写完、`status.json` 还没改成 `done`），所以 `pollAndDeliver` 只在 `status.state == .done` 时才把 report 变成 `task_finished`；report 存在但 status 还没追上，是同一个"这一轮还没定"的分支，跟 report 压根没写一样，下一次 poll 自然会再问（review-fork-remote.md §7.6①：早先按"report 存在就等于完成"投递，会在这条窄窗口里把旧 status 的空 `exit_code` 当成真的，且一旦 `delivered` 落地，后到的正确 `done` 永远不会再被看）。host 侧**任何一个问它的动词**（`task list|status|wait|kill`，以及 `session step` 在它自己那条通道上开步之前的一次扫描）顺手把它翻成 `task_finished` 投进**任务当前 `notify` 指向的那一场**的 inbox。**"哪些任务报告进这一场"只有一份答案**（`cli/task.zig` 的 `collectRows`）：owner 是它的，加上别的 session `task retarget` 过来的（`notify` 从外面读得到，正是为此）——`session step` 的那次扫描就是"跑一遍 `collectRows(only=<本场>)`、把行丢掉"，不是第二份遍历（从前它只走 `sessionTasksDir(<本场>)`，于是 retarget 过来的远端任务永远扫不到）。每个任务的 `cwd` 取它 **owner 场**冻结的工作区，不是发问那一场的。已经开着的那条通道是**借**给这次扫描的（按 spec 匹配，不是按 session——retarget 之后问的是别人的机器），所以本场自己的任务不多付一次连接；owner 在**另一台**机器上时照常连一次，与 `task list` 同价（读者不该比它旁边那个动词答得少）。**driver 看见的东西一个字没变**：仍然是一条在 step 边界排干的 inbox 事件，而不是第二种要认的盘面文件（CLAUDE.md 那条工作约定）。翻译只发生一次——host 在自己那半目录里记一个 `delivered`（`origin` 去重管的是"事件不重复进对话"，而**投递文件重新出现**会让 `depositPending` 永远说"有未读结果"，`wait --any` 于是永远答 0）。

**`task list` 的第五个投影值 `unreachable`**：这台 host 问不到那台机器——通道没开，或者对面拒绝了这次问题。它**不是 `lost`**也**不是 `done`**——什么都不知道，而任务多半还好好跑着。`wait` 撞上它当场结束并点名那台机器（挂死不是答案）。`--json` 因此多一列 `machine`：`log` 那一列此时是**别的机器上的**路径，本机读者打不开，这一列说明为什么。任务在哪运行只来自 owner session 的可读 header；header 丢失或损坏时，`list|status|wait|kill` 响亮失败，**绝不把 unknown 猜成 local**——否则 `kill` 会在 host 写一个远端 supervisor 永远看不见的 marker，却谎报 `kill requested`。

**远端的 `lost` 不再是"问不到的第二个问题"**（review-fork-remote.md §5 修正）：`TaskSnapshot` 多一个可空列 `lease_held`，由对面 `task-poll` 时**顺手**用它自己那份 `leaseHeldIn`（`cli/task.zig`，与本机 `projectState` 同一实现，只是 base 目录换成对面已经打开的工作区句柄）探一次租约；lease 必须是普通文件，显式检查而不依赖 OS 是否允许打开并 flock 一个目录，损坏的 `.lock` 因此是错误而不是 `false`。这一列在**同一轮**里回给 host——不加一次 round trip、也不 bump 协议版本（加一列不用）。`readRow` 的消费规则：状态是 `done` 就是 `.done`；否则 `lease_held == false`（没人守着这个租约）→ `.lost`；`true` 或 `null`（老 peer 没这一列，答不上来）→ `.running`——**不知道就不主张**，这与 `unreachable` 是**答不上来**（连状态本身都没问到）严格分开：`lost` 是"那台机器答了，答案是没人守着这个 supervisor"。**`serveTaskPoll` 读两个文件时的真实 I/O 故障也不再塌成"还没写"**：`readTaskFile` 只把 `FileNotFound` 读成空（= `starting`），别的错误（权限、读越界……）一律 refuse——那次轮询在 host 侧因此落成 `unreachable` 而不是永远 `starting`，是同一条"答不上来不许塌成一个确定事实"的纪律（`Repo.unknown`、`Answer` 收成 union 是先例）。

**`task run` 与 `shell {background:true}` 走同一条路**：前者改建 `launch.sessionEnvironment`（原来是 `localEnvironment` 加一句对 `remote:` 的硬拒），于是"任务跑在它那场 session 跑的地方"在两个入口上是同一段代码。
...
- **exec target 不是权限边界**（§8.1）。把 `shell` 指向一个 WSL 发行版改变的是命令**在哪跑**，不是它**能碰什么**——WSL 经 `/mnt/` 看得见整个工作区。净化这一侧仍然成立（`wsl.exe` 拿到的就是那份剥过 secret 的 map，所以 `WSLENV` 没有 secret 可转发），代价是 `NULYA_EXE` / `NULYA_SESSION` 也到不了对面。（搬整个工作区、且可能经 ssh 到达的那一族是 §8.2 的 `remote:…`——那一侧同样净化 env，且 `SSH_AUTH_SOCK` 在 denylist 上，所以 `remote:ssh:` 用不了本机的 ssh-agent；缺省走密钥文件，显式 `--ssh-password-stdin` 时也可走 host 侧一次性 askpass broker，密码仍不交给远端 agent。）
...
- **workspace store 是 checkout 内容，却是第一优先 root——所以它要被信任一次（trust gate）。** §9.5 把 project 层的 `extensions.paths` 挡在门外，理由是 checkout 不该决定哪些目录供给 `current`；但 `.nulya/extensions` 本身就在 checkout 里，且首个持有者胜（§7.2）。clone 一个带 store 的 repo，从前 `session new` 会机械地把其中 active 版本合进 composition——system_prompts 进 system blocks、tools 经 CLI 可调、配合 project 层允许的 pin 还能上 native 面——中间没有任何人的确认。现在有一道门：
  ...
  - **远端 session 里，那台机器的 workspace root 由它自己门**（§8.2）：`nulya remote serve` 在解析 `run-extension` 之前跑同一份判据，读的是**那台机器**的 trust journal——判据与记录都属于持有字节的一侧，与 entry 变体、`.sealed` 同一条理由。只有拒绝的形状不同：那边没有一场 session 可以拒掉，所以是**一次失败的调用**（点名 store 路径 + 指路在那台机器上 `nulya ext trust`），通道与 session 都不死。
...
- **`extensions/handoff`**（与 `compact` / `evolution` 同层，compiled，理由同 `compact`：要把四个分节当一组校验，而一个 manifest 只有一个 interpreter，随仓库带的东西没法 ps1 + sh 各一份还共用一个 version）contribute 一个 `handoff{done, next_task, keep, drop?}` tool。**只 propose、不 fork**：它不调 `session new`，所以 `session new --parent` 在整个仓库里仍然只被 `extensions/compact/src/main.zig` 调用。它现在只做**一件**事——校验三个必填节（缺 → 一次失败的调用，一次列全缺的），然后回一句"记录好了，别再调工具，结束本轮"。**它一个字节都不写**：四个分节就是这次调用的参数，而**调用已经在 ledger 里**（kernel 在这个进程启动之前就冻好了），磁盘上再写一份就是第二份真相（physics #3）——与 `extensions/plan` 的 `propose`/`todo` 同一条纪律。它也因此不再读 `NULYA_SESSION_ID`：从前要那个 id 只是为了取文件名。
- **从前是一个文件，为什么不是了**（2026-08-30）。旧契约是"往 `.nulya/handoffs/<session>-<n>.md` 写一个文件、driver 去读盘"。三条代价：① 每个 driver 都要学一套没有任何东西强制的目录约定，跨平台还是两份实现；② 那正是 CLAUDE.md 工作约定点名的"driver 要认的新盘面文件"，而它自己就是那条约定说的**历史特例**；③ 工作区一旦住在别的机器上，包跑在远端、文件落远端盘、driver 在 host——`handoff` 因此被显式排除在远端 composition 之外（goals/remote-env.md §3.2）。**提议的全部内容本来就已经在 ledger 里**，真相有更好的家。
- **`extensions/compact` 因此长出 ledger 分支**：`brief=latest`（常用形）或 `brief_seq=<n>`（点名某一次调用）。它 `session events <old>` 读回那一场自己的 ledger，找**最后一次被内核接受的** `handoff` 调用，把它的参数渲染成 markdown brief，其余照 `brief_file` 分支的语义——跳过七步的 2–4、旧 session 逐字节不变、fork 点 = 旧 ledger 当前 tail、carried 文本末尾由代码追加父指针。
  - **判据是"被接受"而不是"存在"**：一次被 `handoff` 拒掉的调用（分节不全）或被 gate 否掉的调用照样在 ledger 里，带着 `ok=false` 的 tool_results；在它上面 fork 就是把刚被否掉的那份 brief 带进下一场。**问的是内核自己的答案**（配对的 `tool_results.ok`），不是在这里把 handoff 的校验规则再实现一遍。
  - **渲染住在 compact 一侧，handoff 只剩校验。** 两个包是两个独立二进制，所以"把四节变成 markdown"必须与"把它 carry 过去"是同一个人，否则同一个形状写两遍、两份会漂开。`handoff` 留下的是只有它做得到的那件事：在调用发生的那一刻告诉模型缺了哪一节。
  - **`brief_seq` 不移动 fork 点**（fork 点恒为 tail）：子场不继承任何 history，所以 lineage 里那个 seq 记的是"这场对话被留在哪里"，不是"从哪里剪断"。三个来源互斥（`brief`/`brief_seq` · `brief_file` · 什么都不给 = 七步路）；拒绝一律干净（没有 events / 没有被接受的 handoff 调用 / 不认识的 `brief` 词 / 参数解不出来（`max_tokens` 截断的 turn，§4）/ 渲染出来是空的）。
...
- **`compact` 的 `brief_file` 分支保留**：给了这个参数就**跳过七步里的 2–4**（不 append 请求、不 step 旧 session，旧文件**逐字节不变**），fork 点 = 旧 ledger 当前 tail（`session events <old>` 的最后一行 `seq`），brief = 文件内容；父一条事件都没有、或文件读不到 / 为空 → 报错不 fork。它今天唯一的 consumer 是 **`extensions/plan` 的 `approve`**（把批准的计划渲染成 brief 文件再 fork；plan 的 brief 走 ledger 化是后续收口）。**每条路径**都由**代码**在 carried 文本末尾追加一段父指针（`Parent session: <id> (forked at seq N) … nulya session events <id>`）——不指望模型记得写；旧 ledger 还在盘上、新 session 有 shell，于是有损压缩退化成惰性检索。
- **fork 不继承后台任务，compaction 继承。** `session new --parent` 对任务一无所知，这是对的：将来的 subagent 也走这条路，而一个子场不该抢走父场的工作。但压缩不是分叉——它是同一场对话换了个文件，把结果投进一个再没人读的 session 就是把结果丢了。所以**继承发生在 `extensions/compact` 里**（两条路径同一段代码，fork 成功之后、carry 之前）：`nulya task list --session <parent> --json` → 每个 `nulya task retarget <task> --to <child>` → carried 文本末尾由**代码**追加一行 `Background tasks still running when this session was forked: <sid>/t3 (<command>, 41s so far) … — nulya task status <sid>/t3; their results will arrive here when they finish.`（与 `parent_footer` 同一手法：模型没法记住一件它从不知道的事）。**retarget 的是每一行，不只是还在跑的那些**：`task retarget` 的另一半是 `moveDeposit`——"结果已经落地、还没人排干"，而那正是 fork 与任务完成之间那个窗口留下的状态，把它过滤掉就是在这个窗口里丢结果。**`.done` 行上这不是"什么都不做"，而是 `taskRetarget` 自己分了两条路**（review-fork-remote.md §7.6④）：先试 `moveDeposit`，只有真的搬走了什么才写 `notify` 指针——一个早已排干、结果被读过的任务不再留下指针，否则它会在往后每一次 compact 里被再指一次，沿 fork 链无限迁移，`/tasks` 的噪音随 compact 次数线性增长而永不停止；非 `.done` 的行仍是"`notify` 先落地、`moveDeposit` 随后"的老顺序（正在跑的 supervisor 完成时要看得见新目标）。**footer 仍只列还活着的**（它承诺的是"结果会到这里来"，一个已经交付过的任务不在那句话里），什么算"还活着"由**内核**回答（行上的 `state`，与 `task list --running` 同一投影，不在这里重算 `lost`）。**retarget 失败绝不让 fork 失败**——stderr 说一句、照常返回，那个任务照旧报告进父场的 inbox，找得到。§6.1 / §14。

**carried 文本在 fork 之前就要验一次 UTF-8**（`session append` 拒绝非法 UTF-8——header 不再是 §3 的形状，BUGS #22）：fork 与 retarget 都不可回滚，验在它们之后会留下一个收不到 summary、却握着父场任务的孤儿 child。所以顺序是**渲染 → 校验 → fork**。同一条不变量的另一半在渲染那一步：每节 64 KiB 的上限**按字符边界裁**（`emit.validUtf8PrefixLen` 的同一条纪律，两个二进制没法共享代码），否则只有长到需要裁的 brief 才会坏。fork 之后才拼出来的只有那句后台任务的 footer——它由代码从任务名与命令生成，坏了也只值一句话，于是单独验、不合格就少一句而不是少一场 session。
- **`drivers/goal.sh` + `drivers/goal.ps1`**（仓库顶层 `drivers/`，各 ≤ 70 行、逐行对齐）是**第一个 driver**，也是 PLAN §3.6 那段伪码的落地：`session new --with handoff@<v>` → `session append` 目标 + 一段"按阶段工作、阶段做完才调 handoff"的前言 → 循环 `session step --max-steps 1 --stream`；每步之后**看这一步自己的流**（`--stream` 的行里出现定长子串 `"tool":"handoff"` → `ext run compact --arg session=<id> --arg brief=latest` → 切到返回的子 id；同样的字节在任何 JSON 字符串字段里都会被转义，所以只有真 key/value 匹配。**刻意宽松**：一次被拒的 handoff 调用也匹配、误报同理，而 `compact` 对这两种都是一次干净的拒绝，driver 把它打到 stderr 照常继续循环），否则看协议里的 `"stopped":"end_turn"` 收工。它**不是 extension**：一个 driver 一跑几十分钟，而 `ext run` 对 extension tool 强制 manifest 的 `timeout_ms`（上限 600s，§7.3）——driver 不是一次 tool call，不该被塞进那个形状；何况 script extension 一个 manifest 一个 interpreter，跨平台就得两个包。两份脚本都**不解析 JSON**：提议的信号是一个定长子串、结束是协议自己的一行、只有一个正则从 compact 的结果里取新 id。`end_turn` 之后还要多问一句 `task wait --any --session <id>`（§14 的三个退出码正是为这一次调用设计的）：**0** = 有后台结果落地了 → `continue` 再 step 一次把它排干；**3** = 没有可等的 → 收工；其余 = 报错。于是"模型说完了"与"这件事做完了"分开——一个还在跑的 `zig build test` 不会让 driver 提前宣布结束。
...
nulya session new [--profile P] [--model ID] [--parent <id>:<seq>] [--with <id>[@<version>]]… [--pin ext:<id>/<tool>]…
          ...
          | prune <id> [--force]                         ← **唯一一个删 session 的动词**。缺省只删得掉什么都没记下的那种（header 一行、
                                                           没有事件——那不是 ledger，只是一个名字；physics #1 管的是历史，这里没有历史），
                                                           前端自动调的就是这一档；`--force` 连**有历史**的一起删（真正的清理是人做得到
                                                           的事，而「这一场不值得留」是判断，所以这个动词只收一个 id、永远不收 pattern）
                                                           它是个动词而不是前端自己 unlink，因为「能不能删」有两条判据是**锁**（有人在
                                                           `step` / 有人正在投递），而锁只能靠**拿**来回答、不能靠看：探测锁的前端恰好
                                                           在最要紧的那一刻猜错——另一个进程正卡在它自己的 check 与 deposit 之间。
                                                           机制在内核（`ledger.pruneSession`：哪些文件构成一场 session、两把租约的编排、
                                                           两个计数；typed error `NoSuchSession`/`SessionBusy`/`DepositInFlight`/
                                                           `HasEvents`/`HoldsDeposits`），检查与删除全程持两把租约（deposit lease 用
                                                           non-blocking：「有人正在投递」是答案不是队列），所以「什么都没握着」这句话
                                                           在据此动手的那段时间里是真的——而它拿的那把是 **inbox 的**租约，投递者一个
                                                           不落地都持它（§3.4），不是两条命令之间的约定：supervisor 送回的
                                                           `task_finished`、`ext activate` 的 capability note，与一条排队的 turn 一样
                                                           是「别动这场 session」的理由
                                                           **`--force` 掀不动的三条**（它管的是这一场*握着*什么，不是谁正握着它）：
                                                           有人在 `step`（写者租约）· 有正在飞的投递 · 这一场还有活着的后台任务
                                                           （壳层用 `task list` 那同一份投影问，refusal 点名 `nulya task kill <task>`；
                                                           `done`/`lost` 不拦——它们的目录随 scratch 一起走）
                                                           删的东西：session 文件（先删，它的不在场就是别人读到的「没了」）· `.cancel` ·
                                                           两个 lease 文件 · inbox（`--force` 连里面排队的一起）· `.nulya/scratch/<id>/`
                                                           **不删的**：两条 journal 的行（「没有行 = unknown」本来就是纪律，§3.3），以及
                                                           `--parent <id>:<seq>` fork 出去的子场（fork 不复制任何东西，照常能跑；只是
                                                           `session list` 的 episode 分组从此连不回那个 root）
...
nulya task run [--session <id>] [--cwd <dir>] [--timeout-ms N] -- <command>
          ...
          | list [--session <id>] [--running] [--json]    ← starting | running | done | lost | unreachable，一行一个
          ...
          | supervise …                                   ← internal：`startShellTask` 起的那个进程，不给人用
                                                           `--session <file>` 与 `--task <sid>/t<N>` 二选一：后者是没有 session 文件的那台机器
                                                           （报告写 `report.txt`、在 `done` 之前，等 host 来取，§8.2）
...
- `session new --parent <id>:<seq>`：这场 session 续的是谁（fork / compaction 的新文件，§11）。**父必须存在**（读不到 header 即 exit 1，不建文件）。模型分两级继承，因为两个 flag 含义不同：`--profile` 换的是"怎么连"，所以它替掉父的 profile；`--model` 只是在一个 profile 内换 id，所以**父的 profile 仍然生效**（不会掉回 `active_profile`）；两个都不给则**原样继承父场此刻在跑的那个身份**（`ledger.scanSession`：父 header 的 `model_identity`，被父场的 rebind 移过去之后的那个——fork 续的是这场对话，那就得续给正在跟它说话的那个模型；否则一次 `/compact` 会把一场 rebind 过的对话悄悄换回创建时的模型，正是冻结身份要防的那件事，而 `extensions/compact` 因此一个字都不必知道 rebind），此时不重解 credential、也不打那条降级警告（继承的身份不会降级为 scripted，缺 key 由需要它的那次 `step` 一次性报响）。composition 一律现解，不继承。**`--env` 缺席时同样继承父 header 的 `environment` 与 `remote_workspace`**（§8.1），命名 `--env`（任意值）则两列都只取 argv；单独给出 `--workspace` 只覆盖目录那一列。`session step --effort E` 是**每次 step 的 generation option**（不是身份，§3）：不给则用 `Config.defaultEffort(header.model, header.model_identity.model)`。
