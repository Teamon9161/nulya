...
**`runner:` 是定义里的一个字段，包内 enum + switch。** 缺省 `nulya`；**认不出的值 warn-and-skip 整个定义**（与坏 frontmatter 同款——一个 persona 悄悄跑在它没点名的 harness 上，比这个 persona 不存在更糟）。五个概念动词 `start / send / pending / stop / drive`，今天只有 `.nulya` 一个 arm；`runner`（与将来的 `runner_version`）在 delegation 开场时**冻进 record**，之后每一轮都按冻结值走（与 session 冻 composition 同一条哲学）。不做 vtable、不做 extension dependency：第二个 arm 会是 stdio 上的 JSON-RPC，没有哪种接口能提前替它准备好；真要把一个 runner 搬出包外时（`runner: ext:<id>`），switch 长一个 arm 就是全部改动。
