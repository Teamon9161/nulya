...
| `plan` | compiled | system prompt + `policy{readonly}` + `commands[/plan]` + `propose` / `todo`（都声明 `readonly`，`todo` 另带 `ui: {render: checklist, panel: true}`）/ `approve`（`audience: driver`）+ `contributes.ui` | mode：`/plan` 或 `session new --with plan --pin ext:plan/propose --pin ext:plan/todo` 戴一场 |
| `ask` | compiled | `ask` tool（声明 `readonly`）+ `commands[/ask]` + `contributes.ui` | 能力不是模式，所以它想常驻：user config `[extensions] with = ["ask"]` + `pinned_native_tools = ["ext:ask/ask"]`；只给一场用是 `session new --with ask --pin ext:ask/ask`（pin 蕴含成员，所以 `--with` 可省） |
