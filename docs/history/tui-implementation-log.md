...
这一节 2026-09-01 之前住在 `docs/tui.md` 的「§11 实施日志」标题下，长到 587 KB——
占了那份文档的 80%，而它记的是**怎么走到今天**，不是**今天是什么**。后者现在写在
`docs/tui.md` 的 §1–§10（设计契约）与 §11 开头那一小节（今天前端是什么样）；这份流水
原样留在这里，因为它偶尔还答得上「这个决定当时是怎么权衡的」。**不必读。**

新的实施日志条目仍写在 `docs/tui.md` §11 里，从 T115 起。
...
### T84 · `/new` 与 `/clear` 拆开成两个概念；`/sessions` 长出一个看得见的新建入口（2026-08-28）

**内核零改动**；`bun test` 新增 3 条（`test/tabs.test.ts`），三张受影响的快照（`views` `sidebar` `overlays`）已重新生成。

**`/clear` 从 `/new` 的别名变成自己的命令。** 两者曾经是同一段代码（`aliases.clear = "/new"`），理由是"这里没有东西会被 clear 掉"——这句话本身没错（ledger 只能 append），但它答的是错的问题：别的 harness 里 `/clear` 与 `/new` 本来就不是一回事，`/new` 开一个新对话窗口，`/clear` 清掉当前窗口重新开始，两个动作落在**哪个 tab**上是不同的。合成一个词之后，nulya 版本的 `/clear` 实际上只做了 `/new` 那一半（另开一个 tab），而 `/clear` 这个词真正在说的那一半——"当前这个位置"——没有任何命令能做。

拆开之后：**`/new` 还是那个**（`ui/App.tsx` 的分支不变，`startDraft`）——另开一个 tab，前面那个原样留着；**`/clear` 是新的**——把**当前** tab 原地换成一张新草稿（`state/tabs.ts` 新增 `TabStore.clear(key, opened?)`：释放这个位置原来的东西——会话 tab 走与 `close`/`replace` 完全同一条 `release()`（自己创建又什么都没说的会话照样被 `discardIfUntouched` 收走，有内容的绝不碰），草稿 tab 无事可放——再在**同一个数组下标**放一个全新的 `DraftTab`。对内核来说这仍然是"开一场新 session"（`materialize` 一字未改），旧 session 的文件、其中的每一条事件，原样留在磁盘上，`/sessions` 照样找得到——变的只是这个 tab 的显示。observer tab（这一步没有自己在驱动、只是在看的会话）用同一个 `clear`：没有为它加特殊拒绝或不同行为，一张新草稿覆盖一个观察位跟覆盖一个驱动位是同一件事。

`commands.ts` 的表头注释是这条改动改得最多的地方——它原来专门解释"`/clear` 为什么不该有自己的入口"，现在要说清楚它为什么**该有**：一个概念一个词，而"另开一个 tab"和"原地重开"是两个概念。`/new` 与 `/clear` 都接受同样的 `[--profile p] [--model id]`。

**看得见、点得着的新建入口。** `TabBar` 的 `+` 只在 `tabs.length > 1` 时画（`<Show when={props.tabs.length > 1}>` 包住整条 bar）——这是每个人开屏第一眼看到的常态,也就是说这个人几乎永远看不到那个 `+`,只能记住要打 `/new`。真正随时可见的两个位置是 `/sessions`（F3 的全屏浏览器）与 docked sidebar（同一个组件 `SessionsView` 的两种呈现），而它们从前只在 `n` 键与脚注的一行小字里提过这件事，没有一行是**可以点**的。`ui/overlays/SessionsView.tsx` 新增 `NewTabRow`——跟列表里每一行同一套视觉语言（`onClick` 的按下松开、`rows.ts` 那套悬停背景），不是一个额外发明的按钮样式，画在标题行与列表之间，两种呈现（`dockedBody()` 与全屏 body）各插一处。理由不是"缺一个控件"，是"看这份列表的人本来就在想下一步该去哪个会话"，`+ new tab` 与已有的每一行问的是同一个问题的另一个答案。

**测试**：`test/tabs.test.ts`（3 条，直接对 `createTabStore` 而不经 `<App>`——同 `test/driver.test.ts` 的先例，Solid 的这几个原语在没有 root 时一样能当纯状态用）——草稿原地换草稿，位置与邻居不变；一个真实创建、已经排进 inbox 的会话被 `clear` 之后文件仍在（`sessionExists`）；两个会话 tab 时 clear 一个，另一个原样待在它的下标上。断言的是机制（同一下标、同一对象引用、文件在不在），不是文案。
### T85 · `/evolve` 一键可发；skill 与包命令重名不再在补全里出现两次（2026-08-28）

**内核一处纯附加**（`src/extension/manifest.zig`）：`{"with": …}` 的值从来就不是只认 `true`——`dupAction` 从一开始就把任意动词的值当字符串收（`{"run": "x"}` / `{"skill": "y"}` 一直是这样），`validate()` 也从没有一条规则只准 `with` 配 bool。这次只加一个读法 `Action.withPrompt()`（`runTarget()` 的同款），把 `{"with": "<text>"}` 里的字符串答出来给需要它的那一层——语法一个字节没改，`kernel_hash` 不变。DESIGN §7.2.1 补了一段说明这一点。

**`/evolve` 现在直接可发。** 以前敲 `/evolve` 只是戴上 `evolution` 包、停在输入框上等，而这个包该打什么并不写在任何地方。现在 `runPackageCommand`（`tui/src/ui/App.tsx`）的 `with` 分支多问一句：命令名后面敲了字（`/evolve 关注 XXX`）就把那段文字当开场消息发出去；没敲字但 manifest 写了默认首条消息（`action.prompt`，来自 `Action.withPrompt()`）就发那句默认；两者都没有，回到原样——只戴、不发，等人说话。`extensions/evolution/extension.json` 的 `evolve` 命令补上了默认："Read your most recent report, then review the sessions since it…"。`plan` / `ask` 的 `with` 动作没写默认，`/plan <content>` 因此**自动**获得同样的"打了字就发"行为——不用改 `plan` 的 manifest，那一半它已经白拿了（"打字覆盖默认"与"没有默认就什么都不发"是同一条规则，不是两条）。
...
**① 行尾一个 `⚙`**（`StatusBar`）：与 `◧ sessions` 对称，**两端是 host chrome，中间是这一场自己的事实**。**是 glyph 而不是词**——T70 的教训不是"永远写全名"，而是 `◧` 对没见过它的人什么都不说、于是那块 pane 没人找得到；齿轮是相反的情形，是不用教就读作"设置"的那一个图标，两格买到九格买到的东西，而且**每个宽度上都在**（≥60 列，与右侧那批 chip 一起让位），不再只在 100 列以上。代价是 §6.3 词表里出现了唯一一个共用形状的条目（`⚙` 同时是 `ext build`），记在表里：两者永不相遇——一个是 transcript 卡的头字形，一个钉在状态行末端，**位置就是区别**，而位置是终端仅有的四样语法之一。前面两列是**空气不是接缝**：`· ` 说"下一个"，空格说"另一类东西"；尾部那一列在点击目标**里面**（把手的同一个理由：一个控件好按比好读更重要）。它也进 `layout()` 的 `right` 预算，所以不会挤掉模型 id。
