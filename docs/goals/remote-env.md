...
3. **远端那个二进制是完整 nulya 还是瘦代理，以及它怎么到远端。** 本文倾向完整 nulya + `nulya remote install <dest>` 一次显式手势；
   ...
   > **一条 ssh 连接共用（2026-09-03，人拍板）**：真实使用里最刺眼的不是连接慢，是**每一次都重连**——
   > 目录浏览器每进一层就是一整条新 ssh（连接 + 认证），`remote check` / `session step` 各拨各的。
   > 起初把 `ControlMaster` 当成一个要请人定夺的取舍（"你机器上会多一条活过命令本身的多路复用连接"），
   > 人的回答是"都远程连接了, 留一条连接很自然吧"。
   >
   > 落地前**先量了三件事**，因为这三件都可能让它变成负收益：(1) 到底快多少——本机 sshd 上三次冷连
   > 0.40s → 三次热连 0.036s；(2) `ControlPersist` 是不是空闲计时器（是：每次复用重置，所以关终端
   > 不需要任何收尾逻辑）；(3) 那个常驻 master 会不会攥着我们的 stdout 写端，让 `runFar` 的 drain
   > 等不到 EOF（不会：OpenSSH detach 时把自己的 stdio 换掉了，管道里 `| cat` 立刻返回）。
   > 第三条是**真会把功能做成挂起**的那种坑，所以没靠推理、用真 sshd 跑了一遍；顺带用 `remote:exec:`
   > 把真通道架在真多路复用 ssh 上验了一次（cold 163ms / warm 36ms，`remote ls` 正常）。
   >
   > **60 秒**而不是十分钟：重拨大约一秒且不需要人（driver 还握着密码），而一条活过工作本身的共享连接，
   > 在一台靠密码登录的机器上正好是一条绕过密码的路。socket 名字用 nulya 自己算的摘要而不是 `%C`
   > ——unix socket 上限约 104 字节，`%C` 多长是实现的事，超限换来的是每次连接一行警告外加照样各拨各的
   > （这个警告在实测里真的出现了，正好确认了那条长度守卫该留）。
