...
**完成**（opus，主工作树，在 W 之后）。六处按 D1–D6 顺序落地，每步 `zig build` 过。**净删 ~600 行内核代码**，manifest 少一个字段，`plain` 契约一个字节没动。

- **D1 · 没有 `wire` 字段了**（`src/extension/manifest.zig` 1759 → 1741）：`Wire` 枚举（含 `fromString`）、`Runtime.wire`、`wireOf`、`ValidateError.InvalidWire` 与 `validate` 里那次检查、`dupRuntime` 里的 `optionalString(…, "wire")` 全删。新增 `Manifest.legacy_wire: ?[]const u8`（`:386`）+ `fn legacyWire`（`:609`）：`parse` 挖进 `runtime` 对象记下写的那个词，**非字符串读成 `""`**（键才是要答的东西，没有词可以引用回去）。唯一读者是 `build/build_ext.zig:47-53` 的两句提示——写 `"jsonrpc"` 的说"那种 wire 已经没有了，这个 runtime 会按 plain 被调用……见 `nulya ext api protocol`"，写 `"plain"` 的说"这个键不再需要"。**为什么两句而不是一句**：`activation` / `permissions` 删掉时作者的意图只有一种，这里有两种——一个人以为自己选了信封，另一个人以为自己选了今天这一种。单测 `manifest.zig:1024` 换成"leftover `runtime.wire` 照建、`ext build` 说得出写的是哪个词"（四个值：`plain` / `jsonrpc` / 认不出的词 / 根本不是词）。
- **D2 · `protocol.zig` = 契约 + 契约里那两条纯规则**（331 → 213 行）：删 `jsonrpc_version` / `method_tool_call` / `ToolCallRequest`（含 `encode`）/ `ErrorBody` / `DecodedResponse` / `DecodeError` / `decodeResponse` / `stringField` / `compactValue` 与它们的 7 条单测；模块注释的 jsonrpc 一节**整个删掉、不留 "deprecated"**（model-facing 文本不讲历史，退场理由留在 DESIGN §7.3）。从 `invoke.zig` 搬进来 `normalizedArguments`（"没有参数就是 `{}`"）与 `PlainEnv` + `isEnvSafeKey`（导出规则：哪些键、NUL 跳过、arguments 必须是 object），**带 6 条新单测**。于是 `nulya ext api protocol` 打印的是契约**和**它的实现，而不是契约加一堆与它无关的 JSON 处理。
- **D3 · `invoke.zig` 只剩 spawn、捕获、失败文本**（673 → 354 行）：删 `request_id`、`Options.wire`、`invokeJsonRpc`、`invokePlain` 这个名字（塌回 `invokeTool` 本身，不再有要分发的东西）、6 条 jsonrpc 单测与只为它存在的 `invokeToolAllocSweep`。**失败文本一个字节没动**，并且从"含有子串"升级成一条 `expectEqualStrings("exit 3\nstderr:\n…\n\nstdout:\npartial work\n")`——e2e（`tests/e2e/std*.zig`）与 TUI 的 `toolSaid` 都按这个形状读，钉死它比钉三个子串诚实。
- **D4 · 调用链**：`extension/tools.zig`（450 → 422）删 `Binding.wire`、`initOwned` 的第六个参数、`call` 里的 `.wire = self.wire`，连带 `ext_manifest` 这个 import 整个不需要了；两条测试合并（"declared wire decides…" 与 "forwards … as a tool/call request" → "forwards the model's raw arguments to stdin"），`error_response` 那个 jsonrpc 信封常量换成 `FakeEnv{.stderr_text, .exit_code}`。`composition.zig:564` 与 `cli/ext.zig:947` 的 `rt.wireOf()` 传参删（`ext run` 那句注释也随之改：它说的是"两条路不能对同一个 runtime 说两种 wire"，而现在只有一种）。`environment.zig` 四处注释改成真话（`ExtensionRequest` 的文档、`request_json` 字段新加一句"这次调用的 arguments 对象"、`env_extra` 那两段不再说"jsonrpc 调用是空的"、写 stdin 那句）；**字段名 `request_json` 保留**——契约允许两种，而改名要动 4 个测试 fake 而不改变任何语义。`extensions/std/src/rpc.zig` 顶部两句 jsonrpc 残句改掉。
- **D5 · 测试**：`tests/e2e/support.zig` 的 `jsonrpc_main_zig` → **`plain_main_zig`**（读 stdin、打一行、exit 0，21 行 vs 原来的 49），`jsonRpcManifestJson` → `fixtureManifestJson`（不再解释它为什么要声明一种 wire）；`extension.zig` 的 `failing_main` 改成 stderr + `std.process.exit(1)`，`jsonrpc_script_ps1/sh` → `greeter_script_ps1/sh`（`cat >/dev/null` + `printf`），`run1.stdout` 那句 `indexOf("greeting")` 改成断言真正的问候语。`script_wire.zig`：**"an unreadable wire word stops the build" 整条删掉**（`InvalidWire` 不存在了），换成 **"a leftover runtime.wire builds and runs, and the build says the key is not read any more"**——真跑一遍那个包，证明老 manifest 不是坏包；两处 `"wire": "plain"` 断言反转成"scaffold 里没有 `wire` 这个词"。`ext_cli.zig` 的 `snooze_ps1/sh` 与 `gate_pin.zig` / `ext_cli.zig` 的 manifest 字符串去掉这个键。**顺带删掉一条早就死掉的引用链**：`src/e2e_support.zig` / `tests/e2e/support.zig` 的 `pub const protocol` 与 `extension.zig` / `session.zig` 里的 `const protocol = support.protocol`——它们只为 `decodeResponse` 存在，grep 后确认零调用。`source.zig` 里 `ext api` 输出断言 `"tool/call"` 改成 `"NULYA_ARG_"`（同一个作用：证明打印的是真源码）。
- **D6 · 文本**：`ext api manifest` 的 `runtime.wire` 整句删掉，改成"内核强制的字段列完之后，加一句『怎么跟 runtime 说话不在 manifest 里，因为只有一种』"；`ext api examples` 的 `--zig scaffolds the same wire` 改说法。DESIGN §7.1 标题「两种 wire」→「一种 wire」、wire 段重写 + 新增一段「`runtime.wire` 是一个已删的键」、`ext init` 那段、§7.2.1 的 manifest 样例与校验清单与"内核强制"那段、§7.3（开头、契约表、jsonrpc 小节整个塌成**一段过去时**「曾经还有一种 wire，叫 jsonrpc……为什么退场……为什么不等一个版本期」）、§7.8 表前一句、§14 的 `ext init` 注与 `ext api` 现状、§15.1 架构图一行。CLAUDE.md：现状段那句「`runtime.wire: "plain"` 是唯一要写的 wire」、Lane W 那条 bullet 末尾接上 D 的记录、模块表 `manifest.zig`（`runtime?{entry, interpreter?}` + 「五个老形状」）与 `protocol.zig`/`invoke.zig` 两行。`extensions/guide` SKILL.md 的 `runtime.wire` 那条删掉（顺带：「a string `result` reaches the model verbatim」是 jsonrpc 的说法，改成「whatever it prints」）；`extensions/evolution` 的 SKILL.md 两处同类残句一并改（它也是 model-facing）。

**一处超出任务清单、但必须做的**：`tui/test/plugin.test.tsx` 的 `plugin` fixture 是一个**说 jsonrpc 的脚本 extension**（manifest 不写 `wire`，脚本读 `params.arguments.text`、回一个信封）。任务说明写的是"TUI 侧不需要改源码"——源码确实没改，但这个**测试夹具**必须迁：`/echo-run` 那条断言等的是 tool 真的收到了参数，而 plain 上它收到的是 `NULYA_ARG_text`。改成三行脚本（`cat >/dev/null` + `printf 'echoed: %s' "$NULYA_ARG_text"`），断言不变。`tui/test/consumers.test.tsx` 顶部一句「they parse JSON-RPC and echo an id back」也改了。TUI 的 `src/` 一个字节未动（grep 确认它从来不读 `runtime.wire`）。

**验收**：`zig build test` exit 0；`zig build e2e` exit 0（88/88）；`zig build && cd tui && bun test` → 365 pass / 1 fail，失败的是 `tasks.test.ts` 的 "an observer never steps, however full the inbox gets"（120s 超时），单独重跑该文件 **11 pass / 0 fail**——`project-nulya-tui-test-gotchas.md` 记过的 lease 等待类既有 flake，与本 lane 无关（第一遍全量跑时它是绿的，绿的那遍失败的是 `plugin.test.tsx`，即上面那个夹具，修好后单独重跑 3 pass）。`bun run typecheck` 干净。`zig fmt --check` 在 LF 副本上对本 lane 改过的 22 个 `.zig` 文件全绿。

**没做 / 留给别人**：`environment.ExtensionRequest.request_json` 没改名成 `stdin_json`（契约明说两种都行，改名只动测试 fake 不改语义）；`docs/goals/ext-review.md` / `ext-review-2.md` / `std.md` / `M2c.md` 与 `docs/history/**` 里的 jsonrpc 叙述是历史契约，未改；`docs/PLAN.md:31/52` 同理（R4 已经判过它们是纯历史记录）。
