...
同一条不变量还有第三个漏口：**外部事实不许让 `render` 造出一个 kernel 随后拒绝的 prompt**。
`git log` 的 commit subject 是这里唯一没有自己上限的字段（`bounded` 允许 4 MiB 输出），
所以让 **git 自己截**——`--format=%h %<(240,trunc)%s`，不再造第二套截断代码，
它给短 subject 补的空格由 `ask` 的 trimEnd 顺手去掉。

**非法 UTF-8 的候选文件跳过，不是失败。** 与 `session new --prompt` 的 UTF-8 契约（DESIGN §14）分工明确：
内核**拒绝**一个不是文本的 prompt 文件（`std.json.Stringify` 会把它写成数字数组而不是字符串，
header 与 provider 请求体两处都不再是该有的形状）；`ground` 这一侧**跳过**它继续找下一个候选——
一个坏 instruction 文件该少一段，不该少一场 session。这也正是 tcode 的 `read_to_string(...).ok()` 的行为。
...
那句话里**必须写出完整的候选顺序** `.nulya/AGENTS.md` → `AGENTS.md` → `CLAUDE.md`：
文件名与优先级**就是这个约定本身**，模型不知道就执行不了它——第一版只写了后两个，
于是 root→cwd 认三个名字、cwd 以下只认两个，两半的约定悄悄漂开了。这**不是**当年 `std` 那版要撤掉的重复：
那版抄的是预算 / fence / UTF-8 截断 / 信任框定 / 发现实现 / 状态，全是**策略**；
三个文件名是**协议**，执行者必须知道。
