...
- **T13 · `@` 文件补全** ✅(2026-08-18)。`tui/src/references.ts`(评分/边界/token 表逐条移植 tcode `composer.rs` + `git ls-files` 索引 + 后台构建/30s 陈旧刷新)、`Composer.tsx` 的 `@` 菜单(`↑↓` 选、`Tab` 上屏、已知引用 accent 高亮)。`bun test` **135 pass**(新增 `references.test.ts` 8 条——其中四条与 tcode 的测试逐条同形——加 `composer.test.tsx` 一条真键盘交互),`src/` 零改动。真跑一遍:在本仓库上索引出 258 条、`@comp` 第三条就是 `@src/composition.zig`、`node_modules` 一条不漏。**偏离**:文件大小只对显示的 8 条 `statSync`(`git ls-files` 不给 metadata,为 20000 条各 stat 一次不值),显示形状与 tcode 一致。无 BLOCKED。
- **T14 · 长文本粘贴折叠** ✅(2026-08-18)。**先探明的结论:OpenTUI 透得出 bracketed paste**(`Renderable.onPaste` + `PasteEvent.preventDefault()`,测试侧 `mockInput.pasteBracketedText` 同一条路),所以**不 BLOCKED**、也没有做任何按键洪流启发式。`tui/src/paste.ts`(阈值 1000 字符 / 15 行、`[Pasted text #N]` 占位、展开、整体 Backspace、占位 accent)+ `Composer.tsx`。`bun test` **140 pass**(新增 `paste.test.ts` 4 条——含 tcode 的阈值边界逐条同形——加 `composer.test.tsx` 一条走真 bracketed paste 的完整往返),`src/` 零改动。图片按 D7 整体不做。**偏离**:attachment 提交后不清空(只随占位被删而走),这样 composer 的 `↑` 历史召回的那行仍解析得出来;生命周期与既有的 `history` 一致。**待手测**:真终端里的粘贴(自动化已覆盖同一条解析路径,剩下的只有终端到 OpenTUI 的那一跳)。无 BLOCKED。
- **T15 · slash skill** ✅(2026-08-18)。`tui/src/skills.ts`(sentinel wrap/parse 照抄 tcode `wrap_skill_echo`、`clipDescription` 100、skill 表缓存与失效)、`cli.ts` 的 `skillList` / `skillLoad`、`Composer` 的补全合并、`App` 的分发兜底、`SkillEchoCard` 的折叠。**修订了两处旧文本**:`commands.ts` 头注释与 `docs/tui.md` §4.4(顺带把 §4.4 的 `@` 与粘贴两条也补上了),`/help` 里那句"nulya has no slash skills" 一并改掉。`bun test` **146 pass**(新增 `skills.test.ts` 5 条 + `render.test.tsx` 一条折叠快照),`zig build test` 绿,`src/` 零改动。真跑一遍:把仓库的 `extensions/guide` 装进一个临时 workspace 的 store → `/g` 补出 `/guide`(描述截断带 `…`)→ `/guide how do extensions work` 变成一条 226 行、带 sentinel 的 user turn(body 里确实有 `nulya help`)→ transcript 折成 `/guide how do extensions work · 226 lines`;`/nope` 返回 null 即原样发给模型。无 BLOCKED。

**四个里程碑全部完成,契约无 BLOCKED,`src/` 自始至终一字未改。**
