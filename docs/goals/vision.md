...
- **V4 done**（`zig build test` + `zig build e2e` 绿；`zig build integration` 无变量时 4 条全 skip 干净）。`wire.dataUri`（两个 consumer：openai 与 codex；anthropic 的 source block 留在自己文件里）。**anthropic**：`writeImageBlock`（`{"type":"image","source":{"type":"base64",media_type,data}}`）接在 text block 之后，`cacheableBlocks` 与 `writeMessage` **按同一条规则**数块——两处都是"有文字才有 text 块 + 每图一块"，所以移动断点在带图时落在最后一张图上（image block 自己能带 `cache_control`）。**openai**：`writeUserMessage` 只在有图时改成 parts 数组，无图仍走原来的 `writeRoleContentMessage`。**codex**：`writeUserItem` 加 `input_image` part。三家都**不为空文本写空 part**（Anthropic 直接拒绝空 text block，而空文本 + 图正是 V3 新造出来的合法 turn）。scripted 未动。新单测 3 条（每个 provider 一条：带图 body 的**整段** user message 逐字符断言 + 不带图的 user message 与改动前逐字节相同 + anthropic 的断点计数与落点，含 image-only turn）。integration 新增第四条：真 64×64 纯红 PNG 的 base64 常量（ledger 存的就是这个形状，不需要编码器）→ 走库路径 append（CLI 门已离线证过，联网口要证的是"我们序列化的字节模型真看得见"）→ 回答里必须有 `red`；**只在本机 catalog 给这个 model id 标了 `vision = true` 时跑**（与门读同一条主张），所以 deepseek 的两个便宜口自动 skip，**codex 与一方 anthropic 口共用这一条**——两者只差一个 `NULYA_INTEGRATION_PROFILE`，抄成两份除了 profile 名一个字不差没有意义。**未做真机实测**：`NULYA_INTEGRATION_PROFILE` 一次都没设过（费用与凭证是用户的），只验证了无变量时 4 条整体 skip、退出 0。**待真 key 实测**：codex 一条 + 一方 anthropic 一条。DESIGN §13（新增一段讲三家的图片序列化 + anthropic 断点那段补"两函数必须逐块同意" + §13.2 的第四条）同 commit；CLAUDE.md「现状一句话」加了一条。
- **默认 catalog 未标任何 `vision = true`**（`default.toml` 一字未动）：契约 D3 的"不猜"同样适用于我们自己写的目录条目——那些 id 的图片支持我无法在本机核实，标错的代价是 provider 400。用户 / TUI 各自 opt-in。这也是 integration 第四条今天会 skip 的原因。
