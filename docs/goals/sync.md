...
- **sync-b**（`db19545`）：`nulya ext sync [--user] [--activate] [--dry-run]`，落点 `cli/ext.zig`（`extSync` / `draftIds` / `appendActivation` / `isDraftFault`）。draft = `<root>/<id>/extension.json`，按名字排序、一个失败不中断其它、host fault 仍照原样传播。行格式 `<id>: <version> <state>[ (copied from <root>)][ 激活尾巴]`，state ∈ `built | already built | not built`，激活尾巴 ∈ `(active) | -> current | (current stays <v-old>)`；另有 `<id>: needs zig (…)` 与 `<id>: failed: <err>`（两者都计入 failed → exit 1）。汇总 `N built, M already built, K failed`（`--dry-run` 首列为 `not built`）。`--dry-run` 走 `build_ext.planExtension`（新的 `Mode.plan`：同一份 manifest / snapshot / 搜索，写之前停手，也不拿 lease）——所以 dry-run 与真跑不可能对同一个 draft 说两样话。填满空 workspace store 时同样记 birth trust（与 `ext build` 同一条 §9 规则）。`BuildResult` 加 `id`（activate 要按 manifest id 而不是目录名）。
  e2e：`cli ext sync: every draft in a root is built in one pass …`（data + script + 从 user root 复制来的 compiled `compact` + 一个坏 manifest → 3 built / 1 failed / exit 1；再跑 3 already built；D2 四场景里的三条＋`(active)`；`--dry-run` 不产生 `versions/`；`--with` 用得上）与 `cli ext sync: a compiled draft with no toolchain and nowhere to copy from says it needs zig, alone …`。
- **sync-c**（`69d5ca4`）：`nulya ext prune [--user] [<id>] [--dry-run]`。删非 `current` 的版本目录（每行 `<id>@<v> removed (<N> KB)`），`current` 缺失的 id 一个都不删并说明原因，汇总后固定打一行"旧 session 无法 resume / 重 build 同源码得同 id"。持 `<id>/.lock`；先只读看有没有版本再拿 lease（否则拼错 id 会凭空建出 `<id>/`）；不合法 id 直接拒。
  e2e：`cli ext prune: every version but `current` goes, an id without one keeps all of them, and --dry-run only says what it would do`（含"重 build 恢复同一个 version id"）。
- **sync-d**（`49540f0`）：`nulya help` 加 `ext sync` / `ext prune` 两行——为了守住 ≤ 40 行把 `ext list` 与 `ext inspect` 并成一行（现正好 40）；`ext api examples` 加一段整 root 的配方；DESIGN §7.2（两个动词的语义 + `--activate` 规则 + prune 的取舍）、§7.4（跨 root 复用 + 为什么匹配键是 seal 三元组）、§14（命令表 + 输出形态 + `--user` 列表）；CLAUDE.md 现状一段 + 两处模块表；guide SKILL.md 的 `Building an extension` 加"最省事的装法"与 prune 的代价（223 行、零文档引用）。e2e 断言扩到 `ext sync` / `ext prune` / `--dry-run` / `--activate`（help）与 `ext sync` / `ext prune`（examples）。
