...
**全部落地（M1–M9），四条验收命令绿。** 未 commit。

- **M1 · 删 `permissions`** ✅ `manifest.zig` 删 `Permissions` struct / `Manifest.permissions` / `dupPermissionList` / parse 那三行；新增 `Manifest.legacy_permissions`（`obj.get("permissions") != null`），唯一读者是 `build_ext.noteLegacyShapes` 的那一行。`ext api` 文本、DESIGN §7.2.1（新一段"**`permissions` 已删**"，含删它的理由）、DESIGN §9 开头那句（`**明确不假装 manifest.permissions 是安全边界**` → `**没有一个 manifest 字段是安全边界**`）、PLAN §3.8 与 M7 里程碑两处措辞。顺带把仓库里仍写这个键的三个**测试夹具**去掉（`extension/notes.zig` 两个、`extension/store.zig` 一个）——deliberate 的老形状覆盖在 `manifest.zig` 的那条新单测里，别处不该再散着抄。
- **M2 · `policy` 只剩 `readonly: ?bool`** ✅ 删 `deny` / `ask` / `InvalidPolicyEntry` / `PolicyAllowNotPermitted`（`ParseError` 因此回到四个成员）；`dupPolicy` → **`readPolicy`**（不再需要 allocator——一个可选 bool 没有要 dupe 的东西）；`validate` 里 policy 那一段整个删掉；`policyContributes` = `readonly != null`。DESIGN §7.2.1 那段重写成"一个字段，而它只能收窄——这两件事现在是同一件事"（形状取代规则）。
- **M3 · `commands[].action` 改成对象** ✅ 新 `manifest.Action{verb, target, keys}` + `runTarget()`；`dupAction` 读两种形状（对象：恰一个键、值是 `true` 或字符串，别的是 `WrongType`；字符串：按第一个空格折成同一对字段并抬 `Manifest.legacy_command_action`）。`validate` 检查 `keys != 1 or verb.len == 0` → **`InvalidCommandAction`**，`run` 的包内引用检查照旧（`UnknownCommandTool`）。**`keys` 这个字段的理由**写在类型注释里：契约要求"恰一个键"是 *validate* 错而不是 parse 错，所以 parse 必须把"写了几个键"带过去。
- **M5 · driver tool 的 `timeout_ms` 删** ✅ `extensions/agent` 的 `render`(120000) / `list`(30000) / `run`(600000)、`extensions/compact`(600000)。`agent` 自己的 120000 留着。DESIGN §7.8 那段"manifest 上的 `600000` 因此只是留着的声明"改写成"三个数字删掉了，它们从来没有生效过"。
- **M6 · `contributes.ui` 按宿主键** ✅ `Ui` → `[]const UiHost{host, entry, api}`（`Manifest.ui` 缺省空表，`NoContributions` 读 `.len`）；`isValidUiHost` 与 `isValidCommandName` 共用 `isLowerDashWord`；`InvalidUiHost` 新增。`dupUi` 折平铺老形（判据：顶层有没有 `entry` 键）→ `tui` 一条 + `Manifest.legacy_ui`。`build_ext.validateUi` 与 `integrity.zig` 三处（`collectPackageSnapshot` / `collectFrozenSnapshot` / `requireDeclaredPaths`）从 `if (m.ui) |u|` 改成 `for (m.ui) |u|`——**每个宿主的 entry 都要在快照里**，理由与 `validateScriptEntries` 同（一个版本服务所有宿主）。`extensions/{plan,ask}` 的 manifest 改成 `{"tui": {…}}`。
- **四个老形状一个提示函数** ✅ `build_ext.noteLegacyActivation` → **`noteLegacyShapes(alloc, io, m)`**（四个 `if` + 一个 `noteLegacyShape(alloc, io, id, what)`）。K 说的"第二个 consumer 到了再决定要不要抽"——第二到第四个一起到了，抽成了**一个函数体里的四行**而不是一张注册表。
- **M7 · `ext api manifest`** ✅ `extApi` 的 topic 判断改成 `manifest` 或 `permissions`（旧名一个版本期、打同一段，`tests/e2e/cli.zig` 断言两者**逐字节相同**）；driver / 前端两层文字按新形状重写（`policy` 只有 `readonly` 且"没有键能拓宽"、`action` 对象形、`ui` 按宿主 + "一个前端读自己那一条"、`timeout_ms` 那段加一句"driver-only 的 tool 因此没有理由写它"）；`examples` 里那段 JSON 片段同步。`common.ext_usage` 的 `api [protocol|manifest|examples]`。
- **M4 · TUI 自动 `/<id>`** ✅ 新 `extensions.derivedCommand(contributions)`：有 system prompt + id 是 `[a-z0-9-]+` + 包自己没声明同名 → `{name: id, action: {with: true}}`；`extensions.packageCommands` 在每个包自己的 commands **之后**推一条（扫描顺序不变），随后走 `packageCommands.resolve` 那条既有的路（内建永不被夺走、同名首个持有者胜）。`extensions/plan` 的 `commands` 条目删掉，`ask` 的留着并改成对象形。
- **M3/M2/M6 的 TUI 一半** ✅ `files.ts`：`PackageCommand.action` 变 `PackageActionValue`（`string | Record`，原样保留）、`PackagePolicy` 只剩 `readonly`、`uiOf` 读 `ui.tui`（顶层有 `entry` 就是平铺老形，仍读成本前端的——**冻结版本留在盘上的字节不会因为 draft 改了写法就变**）、新 `ui_host` 常量。`packageCommands.ts`：`parseAction` 收对象与字符串两形（内部 `splitAction`），`isDeprecatedWearAction` → **`deprecatedActionNote`**（回的不再是 bool 而是要说的那句话——现在有两种老写法而调用点只有一个）。`approvals.ts`：`CompositionPolicy` 只剩 `readonlyBy`、`withPolicy` 删除。`App.tsx`：`decide` 直接拿 `props.style.settings.approvals`（包不再能往三张表里加行），warning 那两行跟着改名。`plugins/host.ts` 只改了三处注释——它读的一直是 `contributions.ui`。`plugin-api.d.ts` 顶部两处。
- **M8 · 文档** ✅ DESIGN §7.2.1（示例 JSON、validate 清单、driver 层两段、前端层三段全部重写；**"三处 `readonly`，并排"整段保留**）· §7.8（`plan` / `ask` 两行 + `agent` 的 timeout 段 + "五个声明位"那段）· §9 开头 · §14（`ext api` 三处 + usage 一行）；CLAUDE.md 模块表 `manifest.zig` 行 + tui-plugin 那条现状 bullet + ext-review 那条 + **新增一条 Lane M 的现状 bullet**；PLAN §3.8 / M7；`goals/tui-plugin.md` 末尾一条"2026-08-23 形状变更"（逐条映射 D1 / D2 / D3 / D10）；tui.md **T49** + §2 文件表那行 + §5 `tui.toml` 注释那行；guide `SKILL.md` 两处 `ext api permissions` → `manifest`；`extensions/plan` 的 README 与 `src/main.zig` 头注释（`/plan` 不再由 manifest 声明）。
- **M9 · 测试** ✅ `manifest.zig`：round-trip 按新形状重写 + 四条新的（action 的"恰一个键 / 值是 true 或字符串"、字符串老形折叠 + `wear` 仍是一个动词、`ui` 按宿主 + `InvalidUiHost` + 空 `{}` 不算贡献、平铺 ui 老形折成 `tui`）+ `permissions` 被忽略一条 + policy 那条重写；`cli/ext.zig` 的 `isManifestFault` 清单补齐七个 command/ui 相关成员（反射派生，加了也只是钉子）。e2e：`cli.zig` 的 `ext api` 那条改成 `manifest` + 断言 `permissions` 别名逐字节相同、needle 换成"每层一个字段"。TUI：`packageCommands.test.ts` 四条重写、`approvals.test.ts` 收成三条（含"包的 policy 永远进不了审批表"）、`extensions.test.ts` 新增 `derivedCommand` 一条、`plugins.test.tsx` 夹具 / `plugin.test.tsx` manifest / `overlays` `pins` `cli` 三处 fixture 跟随。

**越界三处（合并时留意，都是被 M 直接弄假的东西）**：① `src/extension/integrity.zig` 三个 `if (m.ui)` → `for (m.ui)`（M6 换了字段的基数，不改它编译不过）；② `src/extension/build/templates.zig` 顶部注释那句"kernel 解析这个字段但没有读者"（M1 之后不再解析）；③ `tui/src/ui/overlays/ExtView.tsx` 的 `permissionLine` 与它那一行 `<Show>`（连同 `ExtensionEntry.permissions` / `manifestFacts` 的投影）——字段已经不在 schema 里，留着就是在展示一个这个 harness 不再有的概念。§5 把 ExtView 给了 K，但 K 已经合并，且这三处不删就是留下假话。

**一处判断（契约没写死，说明理由）**：M1 的老 manifest 处理，契约 §3.1 只说"parse 当未知键忽略"，但 §6 K 给 M 的那句与本 lane 的任务书都说 M1 复用 `noteLegacyActivation` 那个先例。采纳了后者——一个作者写了 `permissions` 是想声明点什么，静默忽略与 `activation` 是同一种沉默。

**验收（四条全跑，尾巴逐条）**：`zig build test` exit 0（无输出即全绿）· `zig build e2e` exit 0 · `zig build` exit 0 · `cd tui && bun test` **363 pass / 1 fail**（整套跑了四遍，每遍**恰好一条**超时 120 s，而且**每遍是不同的一条、在不同的文件里**：`files.test.ts` 两条各一次、`observer.test.ts` 的 "a refused step flips the role to observer" 一次、`tasks.test.ts` 的 "an observer never steps" 一次——全是 spawn 真二进制并等 writer lease 的那一类），**逐个隔离重跑全绿**（`files`+`observer` 6 pass、`tasks` 11 pass）——`project-nulya-tui-test-gotchas.md` 记过的高负载 flake 类别，与本 lane 无关。`bun run typecheck`（`tsc --noEmit`）干净。`zig fmt` 只对改过的 11 个 `.zig` 文件验过（工作区 CRLF，所以是把 LF 副本拷进 scratchpad 跑 `fmt --check`；只有 `tests/e2e/cli.zig` 的 needle 表因为字符串变短需要重新对齐，已改）。

### Lane T

**T1、T2 已落地，内核零改动**（主工作树，K/C/M 三条 lane 均已 fast-forward 到 main）。未 commit。

- **T1 · `/ext` Enter 只剩一个意思** ✅ `tui/src/extensions.ts` 的 `standingWith`（~347 行）改成 `systemPrompts.length > 0 → false，否则 skills || commands || ui`——原来挤在它前面、被 `derivedCommand` 的文档块插在中间的那段孤儿注释重写并挪到函数正上方。`promptConsequence` 整个删除（连同它的文档块）。`tui/src/ui/overlays/ExtView.tsx`：`switchOn` 对贡献 prompt 的包，notice 从 `promptConsequence(entry.id, true)` 改成 `` `<id>` on · /<id> opens a new tab wearing it for one session · Enter again takes the command away ``；`switchOn` 里 `standingWith(entry)` 判断上方那段注释重写，说清楚为什么 prompt 包永远进不了这个 `if`；`switchOff` 对称地改成 `` `<id>` off · /<id> is gone · versions all stay ``（仍然从 `session_with` 里移除该 id，处理这次改动之前写进去的旧条目）；`modeCell` 的文档块与 detail pane 里那句"a mode · turning it on puts its system prompt in every new session…"一起重写，后者的颜色从 `style.theme.warn` 降成 `style.theme.muted`（不再是警告，是信息）；顶部 import 去掉 `promptConsequence`。`WithPicker.tsx` 未改动（它列的本来就是"有 `current` 且贡献 prompt 的包"，从不依赖 `standingWith`）。
- **T2 · 开屏只问一句** ✅ `tui/src/extensions.ts` 新增：`store_choices`（从 `promptText` 内联数组提出来，`promptText` 改读它）、`agents_choices`、`both_choices`、`no_store_action`、`CheckoutAction`/`CheckoutAsk`/`CheckoutPlan` 三个类型、纯函数 `planCheckout(store: ProjectStorePlan, agents: AgentTrustPlan)`（按 `type` 从 `agents.ts` 导入 `AgentTrustPlan`，不产生值层面的循环依赖）+ 两个内部文本组装函数 `agentsPromptText`/`bothPromptText` + `applyStoreAction`（`applyAnswer` 改成薄薄一层调它）+ `checkoutFollowUp`。`tui/src/main.tsx`：`askAboutProjectStore`/`askAboutProjectAgents`/`readAnswer` 三个函数删除，合成一个 `askAboutCheckout(ws, syncOnStart)`——`syncOnStart` 为 false 时 store 半句强制 `{kind:"none"}`（复刻原来 `settings.extensions.sync_on_start ? … : "none"` 那行三元的语义），agents 半句不受它影响；读键循环复用 `readKey()`，直到 `plan.apply(key)` 接受一个答案；两处 `rememberStoreAsked`/`rememberAgentsAnswer` 分别只在对应那一侧真的问了（`storePlan.kind === "ask"` / `agentsPlan.kind === "ask"`）才调用。两处调用点（原第 111、118 行）合成一行 `const { projectStore, agentsTrusted } = await askAboutCheckout(ws, settings.extensions.sync_on_start)`。
- **T3 · 测试与文档** ✅ `tui/test/extensions.test.ts`：`standingWith` 那条测试改名重写（prompt 包现在断言 `false`；skill/commands/ui 三种各自仍是 `true`；新增一条"prompt + skill 同时贡献仍是 `false`"钉住"prompt 优先"这条规则）；`promptConsequence` 那条测试整条替换成两条新的——`planCheckout`（六种组合：都不问、都 ready、只 store、只 agents、两者都问时的三个答案，逐项断言 `text`/`choices`/`apply` 的返回值，并断言"只一边问"时的 `text` 与今天的 `promptText`/agents 问句逐字节相同）与 `checkoutFollowUp`（四种"谁被问了、谁做了什么"的组合）；"what a built version contributes…" 那条里遗留的 `expect(standingWith(what!)).toBe(true)` 改成 `false`（这个夹具本身就是一个纯 prompt 包）。`tui/test/overlays.test.tsx` 的 `/ext marks a package that contributes a system prompt as a mode…` 整条重写：断言 Enter 之后 `session_with` **不含**该 id、notice 含 `/house.style` 与"Enter again takes the command away"、detail 文案含"nothing here composes it standing"；OFF 之后断言 notice 含"/house.style is gone"。`tui/test/agents.test.ts` 未改（`planProjectAgents` 本身的行为一字未动，只是多了一个新调用方）。
  - **运行中发现并修的一处遗漏**：`tui/test/consumers.test.tsx` 里对真实 `extensions/plan`/`extensions/ask` 两个自带包的 `plan is a mode and ask is a capability, and their shapes say which` 那条，原先断言 `standingWith(plan)` 为 `true`——这是真机 e2e，`plan` 真实清单里**同时**有 `system_prompts` 与 `ui.tui`（评审面板），正是"贡献 system prompt 之外还贡献别的东西"的活例子。按新规则改成 `false`，并在测试注释里写清楚为什么面板不受影响：`/plan` 打开的那一场自己带 `--with plan`，插件从那一场自己的 composition 加载，从不依赖这个前端的标准 `session_with`。这一条不在原始文件归属清单里（K/C/M 三条 lane 都没碰过它），但它验证的正是 T1 改的那个函数，所以在 Lane T 范围内修了。
- **CLAUDE.md**：ext-review-2 Lane K 现状 bullet 末尾 `TUI 侧见 tui.md T47` 改成 `TUI 侧见 tui.md T48/T50`（T47 说的是 `/sessions` 那张表的一行，从来不是这条规则住的地方；这是一处既有的错误指路，顺手修了）。
- **docs/tui.md**：§5.3 的 `Enter` 小节三条 bullet（ON / OFF / "一个 contribute 了 system prompt 的包是模式"）按新行为重写；§5.10 的 trust 问句那条 bullet 改写成"与 store 问句共用一次询问、两边都问时合成一句"；新增 §11 **T50**。§7（`tui.toml` 那段提到 `extensions.autoActivatable` 的句子）**故意未动**——那个字段已经在 T48 被删，这句话在 T50 之前就已经是错的，但它不在本 lane 归属的 §5/§11 范围内，改了会跟 §5 归属表说的"不是你的文件就别动"冲突；留在这里点名，供下一次路过的人处理。

**验收**：`bun run typecheck`（`tsc --noEmit`）两遍都干净（无输出）。`bun test` 完整跑两遍——第一遍 **364 pass / 1 fail**（`consumers.test.tsx` 的 `plan is a mode and ask is a capability…`，改规则前的旧断言，见上面"运行中发现并修的一处遗漏"）；修完那条测试后重跑一遍全量：**365 pass / 0 fail，32 snapshots，2313 expect() calls，38 个文件，174.85s**，exit 0，本 lane 没有触发任何一条既有的 lease/超时 flake（K/C/M 三条 lane 各自的验收记录里都提到过这一类高负载超时，本次全绿）。`zig build test` / `zig build e2e` 未跑——本 lane 内核零改动，Lane K/C/M 各自已经跑过并记在自己的 §6 小节里。
