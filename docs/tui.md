...
- **ON** = `ext activate <id> <version>`（版本取 sync plan 说 built 的那个，否则 store 里最新的 build；一个都没有就拒绝并指向 `b`）**+** 把它声明的 **model-audience** tool 进本 TUI 的 pin 列（`extensions.pinsOf`，判据是冻结 manifest 的 `audience`，DESIGN §7.2.1）**+ standing with**（T48）：这个包贡献了 skills / system_prompts / commands / ui 任一时，把 id 写进 `tui-state.json` 的 `session_with`，`session new` 时逐个 `--with`。第三样是 2026-08-23 加的，因为 `activate` 从此只说 `<id>` 指哪个版本、composition 一点不改（DESIGN §5.1）——没有它，一个 mode 的开关就是移一个指针、屏幕上什么都不发生。纯 tool 包不写：它的 pin 自己就把包带进来了。
- **OFF** = 先把它的 tool 从本 TUI 列**和 user config 的 `always`** 里撤掉（别的 config 层写的撤不了，点名说出来）与 `session_with` 里的那条撤掉，再 `ext deactivate`。顺序是有意的：pin 或 `with` 指着一个没有 `current` 的 extension，`session new` 是**整场拒绝**（pin 蕴含成员、而没有 `current` 可带，`WithVersionNotFound`），所以先撤引用、后撤指针，中间任何一刻的世界都是合法的。`ExtView.dropOrphanPins` 与 `App.healStandingPins` 修的是已经写坏的文件。
- **一个 contribute 了 system prompt 的包是"模式"，开它要把后果说出来**（T31；T48 收成一档）：id 列表上多一格 `mode`（开着时 warn 色：开它 = 这个前端此后每一场都戴），Enter 的 notice 是 `extensions.promptConsequence` 的一句 `<id> active · its system prompt now enters EVERY new session from this front end · /with <id> wears it for one session instead · Enter again to turn it off`。T37 起曾经分两档（`mode` / `opt-in`，看包自己声明的 `activation`），T48 随那个字段一起收回：reach 不是包说了算的，这一行 Enter 才是。detail 里那句"composed into every session"另说得出是**哪张单子**放的（config 的 `[extensions] with` / `tui.toml` 的 `session_with` / 这个面板自己）。
