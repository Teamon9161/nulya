...
| D10 | **给人用的：一切在屏幕上完成** | 启动 `nulya` 之后，选模型 / 换 effort / 看哪个 profile 缺 key / **贴 key** / **给一个 profile 配档位** 都是屏幕上的交互（`/model` 选择器与它的 `s`、`/effort`、`/provider` 与它的 `s`），**不能要求人去找 config 文件改**。TUI 记住上次的选择（`tui-state.json`，见 §7）；隐式的选择跑不了（缺 key）时开屏就是选择器 + 原因 + 怎么修。config 文件是**定义**（一个 model id 是什么、profile 怎么连）不是**日常操作面** | 这是 TUI 的关键设计理念，与 D4 分工：`tui-state.json` 只有程序写；`tui.toml` 与内核 `config.toml` 是**人写的文件**，TUI 只对它们做**最小编辑**——在末尾追加/就地替换一个带标记的 `[[provider.profiles]]` 小块（`nulya/credentials.ts`：一个 profile 的 `api_key`、一整个人加的 endpoint、或者它一档 `roles` 的一条——**一档一个块**，所以改 `explore` 指向哪里动不到 `review`），以及 `/settings` 里就地换掉一个键所在的那一行（`state/settingsfile.ts`，T100）；两处都不重写、不重排、不碰人的注释与顺序，所以文件仍是同一份文档、作者仍只有人一个。「不能要求人去找 config 文件改」正是这条规则要禁的事，一个只读的设定面板违反的就是它。内核不学"上次选了谁"（那不是 substrate）；kernel 只提供 `nulya config show --json` 一个投影（含 `paths`），TUI 不复刻配置合并链、不猜 home 在哪 |
...
| `nulya session append <id> --file f` | 发送：写 `.nulya/scratch/tui-<nonce>.txt` 再 `--file`（多行 / Windows 引号安全）；投进 inbox，**下一 step 边界才进 ledger**（PLAN §4 边角）→ TUI 乐观回显、标 `queued`，见到对应 `user_text` 事件后转正——那条事件行现在在 `model started` **之前**就到（DESIGN §14，T27），所以 `queued` 只在真正还排着队的时候挂着，而不是整整一个 step。stdout 印一行投递名回执（DESIGN §14），`session.ts` 就按它转正：`enqueueUser` 先挂一个还不知道自己名字的回显，append 回来 `confirmQueued(localId, delivery)`，事件行的 `origin` / `origins` 里出现这个名字才把它摘掉。**不按文本匹配**——两条一样的 "hello" 是两次投递，别人的同文本 turn 也不该顶掉我们的。回执比事件晚到那一格（step 先排干了 inbox）由 `confirmQueued` 自己收尾 |
...
- `/model`（F5）与 `/provider`（F6）是**两个命令、两个问题**（T5 → T6 → T20 → T21，与 tcode 的 `/model` ÷ `/provider` 同一刀）：
  - `/model` **只有模型**：provider 是**标题**（说一次，底下都是它的），每个 model 一行（`label · id · ctx · ‹ effort › · @档位 · ✓ current`），`h/l` 拨 effort、Enter 开新场、**`s` 把一个 sub-agent 放到这一行上**：它**问的是"谁"，不是"叫什么名字"**——按下去弹出这台机器上发现的 persona 列表（`ext run agent list` 的 `rung` 列，与 `/agent` 同一次读，且只在按键时才花那个子进程），选一个 Enter。没人记得自己没写过的档位名，所以这里永远是一张可选的表、不是一个要打字的框；一个 persona 都不问模型时它只说一句并指向 `/agent`。写进去的是**当下这一场所在 profile** 的一档（不是光标路过的那个 provider 的——档位表挂在主模型跑的那个 profile 上）。跑不了的 provider 不出模型行（这才是让表变短的东西）；**离线替身也不出**——它不是谁会挑的 endpoint，只有它正是当下这一场跑着的那个时才出现（一个 key 都没有时 `launch.planLaunch` 就把人放在它上面，那时它必须看得见）。一个 model 的 ctx / effort 档位**先读该 profile 自己的 catalog**、没有才回落全局 `[[models]]`——同一个 id 在订阅口与公共 API 口是两个东西。**光标落在别家 provider 的行上按 `s`，写出去的值就带上那个 provider**（`"explore" = { model = "deepseek/deepseek-v4-flash" }`——档位名总是引号包着写成 TOML key，因为 persona 名字里可以有 `.`，裸键会被读成一张表）——主模型 gpt-sol、explore 用 deepseek flash 是一个正常的队伍，不是一种错配。当下这支队伍画在**它落到的那些行**上（`@explore`），所以 `s` 存完当场就看得见谁跑在这，包括跨到另一家的那一档。一个 provider 都跑不了时只有一行 `no provider can run yet · /provider …`，Enter / `p` 就是过去。
