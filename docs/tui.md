...
- **ON** = `ext activate <id> <version>`（版本取 sync plan 说 built 的那个，否则 store 里最新的 build；一个都没有就拒绝并指向 `b`）**+** 把它声明的 **model-audience** tool 进本 TUI 的 pin 列（`extensions.pinsOf`，判据是冻结 manifest 的 `audience`，DESIGN §7.2.1）**+ standing with**（T48；T1 收窄，ext-review-2 §3b）：这个包贡献了 skills / commands / ui 任一时，把 id 写进 `tui-state.json` 的 `session_with`，`session new` 时逐个 `--with`——**贡献了 system prompt 的包永不写这一条**，见下一条。第三样是 2026-08-23 加的，因为 `activate` 从此只说 `<id>` 指哪个版本、composition 一点不改（DESIGN §5.1）——没有它，一个纯 skill/命令包的开关就是移一个指针、屏幕上什么都不发生。纯 tool 包不写：它的 pin 自己就把包带进来了。
- **OFF** = 先把它的 tool 从本 TUI 列**和 user config 的 `always`** 里撤掉（别的 config 层写的撤不了，点名说出来）与 `session_with` 里的那条撤掉（若有——一个模式从未被写进去过，这一步对它是空操作），再 `ext deactivate`。顺序是有意的：pin 或 `with` 指着一个没有 `current` 的 extension，`session new` 是**整场拒绝**（pin 蕴含成员、而没有 `current` 可带，`WithVersionNotFound`），所以先撤引用、后撤指针，中间任何一刻的世界都是合法的。`ExtView.dropOrphanPins` 与 `App.healStandingPins` 修的是已经写坏的文件。
- **一个 contribute 了 system prompt 的包是"模式"，Enter 对它的意思和对别的包完全一样——让它可用**（T31 点名它、T48 一度把它并进 standing with、**T1 把那半撤回**，ext-review-2 §3b）：id 列表上仍有一格 `mode`（信息性，不是警告——这一行不再说"开它 = 每场都戴"）。Enter 只移 `current`（+ 它声明的 model-audience pin，如果有），**从不写 `session_with`**：`extensions.standingWith` 一处判断，`systemPrompts.length > 0` 直接 `false`。换来的是 M4 派生的 `/<id>` 命令（`extensions.derivedCommand` 读 `current`）：Enter 的 notice 是 `` `<id>` on · /<id> opens a new tab wearing it for one session · Enter again takes the command away ``，OFF 对称地说 `` `<id>` off · /<id> is gone · versions all stay ``——两句都不再提"EVERY session"，因为这一行不再造成那件事。detail 里那句"a mode"的说明也从"turning it on puts its prompt in every new session"改成"Enter gives it a `/<id>` command that wears its prompt for one session"。**一个模式常驻每一场仍然可以，只是不再是这一行 Enter 的事**：config `[extensions] with` 一行，或 `tui.toml` 的 `session_with`——都是人写的、显式的、罕见的决定；detail 里另一句"composed into every session"（`composedEverySession`）照旧点名是**哪张单子**放的，与这一行的 Enter 完全无关。
