...
- 有 tool call 在等批准时（§5.7），`y` / `A` / `a` / `n` / `N` 先于一切（内核这会儿停在那一个 call 上），且只在**输入框是空的**时候——它们首先是字母；`N` 之后输入框收下的那一行是拒绝理由，`Esc` = 不写理由地拒。
...
- **问句在输入框上面，标记在卡片上**（T27 推翻"卡片多一行"）：`ApprovalPanel` 一行一个答案（`y` 允许 · `A` 连同本批剩下的一起允许 · `a` 允许并本场不再问这一类 · `n` 拒绝 · `N` 拒绝并打一行理由——回车送出、模型读得到，Esc = 不写理由地拒），**每行一个字符串、由我们 `fit` 到宽度**，鼠标点行 = 按那个键；那张 tool 卡上留一行 `waiting for you — answer below`，说的是**哪一个** call。原来的理由（"命令已经画在上面了，再画一个框是第二种视觉语言"）漏了两件事：一个 turn 的**整批** call 在第一个执行之前就全画出来了，所以内核停在的那张卡通常**不是**最后一张，问句夹在中间；而那一行是八个 text 节点排成的 row，窄于 78 列就被终端从任意位置折断成两行乱码（`run this?` 与键名互相插进对方中间）。答案给在哪里，问题就该问在哪里——与 §5.8 的 handoff 面板同一个位置、同一条理由。
- **一批 call 的批量答复**（T27）：内核的 gate 天生是串行的（call N 只在 N-1 跑完之后才问，DESIGN §4），所以这里没有 tcode 那种"一个对话框审一整批"的位置；等价物是 `A`——**人看得见的那些 call**（整批都已经是屏幕上的卡片）一次答完。实现是把那批**尚未执行**的 `call_id` 记进一个集合，后续请求逐个消费；不是一个布尔，否则一次 run 里的**下一批**（还没人看过）会被它悄悄盖住。它排在 `[approvals] deny` 之后：一条"永不"不该被一次关于六个 call 的按键推翻。面板抬头写 `1 of 3 in this batch`，只有一个 call 时这两样都不出现。
